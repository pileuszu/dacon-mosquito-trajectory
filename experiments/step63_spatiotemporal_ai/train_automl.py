...
def main():
    ...
    send_discord_notification(None, f"🚀 Started: [Step 63 train_automl.py] Training 5-fold Spatiotemporal model with alpha=200...")
    
    try:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        data_dir = Path("step63_spatiotemporal_ai/data")
        ...
        models_dir = Path("step63_spatiotemporal_ai/models")
        ...
        success_msg = f"✅ Finished: [Step 63] OOF 1-Step: {overall_hr_1*100:.3f}%, 2-Step: {overall_hr_2*100:.3f}%"
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [Step 63]\nError: {str(e)}\n\n{traceback.format_exc()}"
