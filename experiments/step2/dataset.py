...
class MosquitoPhysicsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Base Coordinates (10, 3)
        xyz = df[['x', 'y', 'z']].to_numpy(dtype=np.float32)
        
        # 2. Feature Engineering: Velocity and Acceleration
        # Velocity (v = p_t - p_{t-1})
        vel = np.diff(xyz, axis=0, prepend=xyz[0:1]) # (10, 3)
        
        # Acceleration (a = v_t - v_{t-1})
        accel = np.diff(vel, axis=0, prepend=vel[0:1]) # (10, 3)
        
        # Concatenate features (10, 9)
        features = np.concatenate([xyz, vel, accel], axis=1)
        
        # 3. Normalization (Translation invariance)
        # Shift all coordinates relative to the last known point
        last_xyz = xyz[-1]
        ...
        if not self.is_test and self.labels_df is not None:
            ...
            # Target is the residual relative to CV Prior
