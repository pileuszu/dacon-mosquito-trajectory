...
from step26_hybrid_smoothing.physics import make_candidates, extract_smoothed_derivatives, EPS

# Biomechanical Centroids (Typical profiles from EDA)
...
def extract_context_features(xyz):
    ...
    # Trajectory-level Genus Modality Features (continuous distance to behavior profiles)
    ...
    # 2. Extract smoothed derivatives using W5 Polyfit
    smooth_ctx = extract_smoothed_derivatives(xyz, window_size=5)
    
    return {**raw_ctx, **smooth_ctx}

def prepare_step26_data():
    ...
    print("Preparing Step 26 Hybrid + Smoothed Ranker Data...")
    ...
    for fid in tqdm(sample_ids):
        ...
        # s4 and s7 priors are appended at the end of cands_list
        num_physical = len(cands_list) - 2
        
        if num_physical <= 50:
            # Slow: Keep all 22 candidates
    ...
    out_path = Path("step26_hybrid_smoothing/train_ranker_v26.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 26 Data saved successfully to {out_path} ({len(out_df)} rows)")
