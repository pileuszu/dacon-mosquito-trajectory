...
from step30_feature_decoupling.physics import make_candidates, extract_multi_scale_derivatives, EPS
...
def prepare_step30_data():
    ...
    print("Preparing Step 30 Decoupled Features Ranker Data...")
    ...
    out_path = Path("step30_feature_decoupling/train_ranker_v30.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 30 Data saved successfully to {out_path} ({len(out_df)} rows)")
