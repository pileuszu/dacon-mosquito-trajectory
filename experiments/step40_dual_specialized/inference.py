...
from step40_dual_specialized.physics import (
...
def run_step40_inference(batch_size=250, blend_priors=True):
    try:
        msg = f"🚀 [Step 40] Tri-Model Soft-Blending Inference Started (batch_size={batch_size}, blend_priors={blend_priors})..."
        ...
        predictor_cruising = TabularPredictor.load(str(models_dir / "ranker_v40_cruising"))
        predictor_gliding = TabularPredictor.load(str(models_dir / "ranker_v40_gliding"))
        predictor_steering = TabularPredictor.load(str(models_dir / "ranker_v40_steering"))
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for fid in batch_ids:
                ...
                # Generate candidates using the most probable GMM cluster setup
            ...
            # Vectorized predictions from all 3 predictors with GMM-probability masking to speed up inference
            ...
            # Post-process with Anisotropic Spatial Probability Blending and Soft Routing
            for info in batch_info:
                # Blend model prediction scores using GMM probabilities
        ...
        out_path = Path(f"outputs/step40_dual_specialized/submission{suffix}.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 40 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 40] Tri-Model Soft-Blending Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 40] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 40 Tri-Model Soft-Blending Inference")
    ...
    run_step40_inference(batch_size=args.batch_size, blend_priors=not args.no_blend)
