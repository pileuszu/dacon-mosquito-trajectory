...
def visualize_validation_paths(model, X_val, y_val, mean_stats, std_stats, device, out_dir):
    ...
    if len(steering_indices) == 0:
        # Fallback to any indices if no steering found
    ...
    for idx in selected_indices:
        ...
        ax.scatter(pred[0], pred[1], pred[2], color='#ff7f0e', s=80, label=f'Frenet ODE Pred (Err: {err_cm:.2f}cm)', marker='s', zorder=4)
        ...
        ax.set_title(f"3D Path: Frenet Neural ODE (Sample {idx})", fontsize=13, fontweight='bold')
        ax.set_xlabel("X (m)")
        ax.set_ylabel("Y (m)")
        ax.set_zlabel("Z (m)")
        ax.legend(frameon=True, facecolor='white', shadow=True)
        
        plot_path = out_dir / f"val_trajectory_sample_{idx}.png"
...
def main():
    ...
    send_discord_notification(
        None,
        f"🚀 Started: [Step 50 train_automl.py] Training 5-fold Frenet-Guided Neural ODE...\n"
    ...
    try:
        ...
        # Load dataset
        data_dir = Path("step50_frenet_ode/data")
        ...
        models_dir = Path("step50_frenet_ode/models")
        ...
        for fold in range(folds_to_train):
            ...
            model = FrenetNeuralODEModel(input_dim=24, latent_dim=64).to(device)
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                for Xb, yb in train_loader:
                    ...
                    # Extract features & is_steering mask & initial acceleration
                    ...
                    # lambda = 1e-5 for Steering, 1e-3 for Cruising/Gliding
                    ...
                    loss = soft_hit + 126.309 * huber + reg_loss
                    loss.backward()
                    optimizer.step()
            ...
            # Generate 3D visualization plots for validation paths on Fold 1
            if fold == 0:
                print("\nGenerating 3D validation trajectory plots to verify Frenet ODE performance...")
                val_vis_dir = Path("outputs/step50_frenet_ode/val_visualizations")
                # Reload best fold 0 model weights for plotting
                best_model = FrenetNeuralODEModel(input_dim=24, latent_dim=64).to(device)
        ...
        if args.fold_limit is None:
            ...
            np.save("step50_frenet_ode/oof_predictions.npy", oof_preds)
            print("Saved OOF predictions to step50_frenet_ode/oof_predictions.npy")
            
            send_discord_notification(
                None,
                f"✅ Finished: [Step 50 train_automl.py] Frenet-Guided Neural ODE Training Completed successfully!\n"
            ...
            send_discord_notification(
                None,
                f"✅ Finished: [Step 50 train_automl.py] Dry Run Completed Successfully!\n"
                f"Fold scores: {[round(s*100, 3) for s in fold_hr_scores]}"
            )
            
    except Exception as e:
        error_msg = f"❌ Failed: [Step 50 train_automl.py]\nError: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
