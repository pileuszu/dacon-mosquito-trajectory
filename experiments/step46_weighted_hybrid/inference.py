...
from step46_weighted_hybrid.physics import (
...
from step46_weighted_hybrid.prepare_data import extract_context_features, CLUSTER_FEATURES, REGIME_MAPPING
...
def run_step46_inference(batch_size=250, blend_priors=True):
    try:
        msg = f"🚀 Starting: [Step 46 inference.py] Batched test inference (blend_priors={blend_priors})..."
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            pred_dict = {
                ...
                "cand_disp_normal": np.concatenate(batch_grid_scale), # placeholder normal/binormal will be recalculated if needed, but we follow step45 exactly
            ...
            # Fix normal mapping to follow correct feature key
            pred_dict["cand_disp_normal"] = np.concatenate(batch_cand_disp_normal)
            ...
            for info in batch_info:
                ...
                if blend_priors:
                    # These will be updated with optimized weights from optimize_blending.py
                    optimal_weights = {
        ...
        out_path = Path(f"outputs/step46_weighted_hybrid/submission{suffix}.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 46 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ Finished: [Step 46 inference.py] Submission created successfully!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ Failed: [Step 46 inference.py] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 46 Hybrid Inference")
    ...
    run_step46_inference(batch_size=args.batch_size, blend_priors=not args.no_blend)
