...
from step46_weighted_hybrid.physics import make_candidates, extract_multi_scale_derivatives, EPS
from step46_weighted_hybrid.prepare_data import extract_context_features, REGIME_MAPPING
...
def main():
    print("=== Step 46: Optimizing Hybrid Classifier-Regressor Blending Weights on Unbiased OOF Validation ===")
    ...
    print("Loading Step 46 OOF predictions...")
    oof_df = pd.read_csv("step46_weighted_hybrid/data/oof_predictions_final.csv")
    ...
    for fid in tqdm(unique_ids):
        ...
        is_classification = gmm_cluster in [0, 1, 3]
    ...
    print(f"\nStep 46 Baseline (Anisotropic Smoothed only, no blending) Overall Hit@1cm: {overall_raw_hit:.4%}")
