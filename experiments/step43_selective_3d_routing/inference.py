...
from step43_selective_3d_routing.physics import (
...
from step43_selective_3d_routing.prepare_data import extract_context_features, CLUSTER_FEATURES, REGIME_MAPPING
...
def run_step43_inference(batch_size=250, blend_priors=True):
    try:
        msg = f"🚀 Starting: [Step 43 inference.py] Batched test inference (blend_priors={blend_priors})..."
        ...
        # Load 4 Specialized AutoGluon Predictors
        print("Loading 4 Specialized AutoGluon Predictors...")
        models_dir = Path("step43_selective_3d_routing/models")
        ...
        predictor_steering_mod = TabularPredictor.load(str(models_dir / "final_steering_moderate"))
        predictor_steering_ext = TabularPredictor.load(str(models_dir / "final_steering_extreme"))
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for fid in batch_ids:
                ...
                # Map to 4-Regime soft probabilities
                p_cruising = float(probs_6[1])
                p_gliding = float(probs_6[0] + probs_6[3])
                p_steering_mod = float(probs_6[2] + probs_6[5])
                p_steering_ext = float(probs_6[4])
                ...
                batch_info.append({
                    ...
                    "p_gliding": p_gliding,
                    "p_steering_mod": p_steering_mod,
                    "p_steering_ext": p_steering_ext,
                    "speed": speed,
                    "tangent": tangent,
            ...
            scores_sm = np.zeros(len(pred_data), dtype=np.float32)
            scores_se = np.zeros(len(pred_data), dtype=np.float32)
            
            mask_c = np.zeros(len(pred_data), dtype=bool)
            mask_g = np.zeros(len(pred_data), dtype=bool)
            mask_sm = np.zeros(len(pred_data), dtype=bool)
            mask_se = np.zeros(len(pred_data), dtype=bool)
            ...
            for info in batch_info:
                ...
                if info['p_steering_mod'] > 0.001:
                    mask_sm[info['start'] : info['end']] = True
                if info['p_steering_ext'] > 0.001:
                    mask_se[info['start'] : info['end']] = True
            ...
            if np.any(mask_sm):
                proba_sm = predictor_steering_mod.predict_proba(pred_data[mask_sm])
                score_col_sm = 1 if 1 in proba_sm.columns else proba_sm.columns[0]
                scores_sm[mask_sm] = proba_sm[score_col_sm].values
                
            if np.any(mask_se):
                proba_se = predictor_steering_ext.predict_proba(pred_data[mask_se])
                score_col_se = 1 if 1 in proba_se.columns else proba_se.columns[0]
                scores_se[mask_se] = proba_se[score_col_se].values
            
            # Anisotropic Blending
            for info in batch_info:
                sub_scores_c = scores_c[info['start'] : info['end']]
                sub_scores_g = scores_g[info['start'] : info['end']]
                sub_scores_sm = scores_sm[info['start'] : info['end']]
                sub_scores_se = scores_se[info['start'] : info['end']]
                
                probs = (
                    info['p_cruising'] * sub_scores_c +
                    info['p_gliding'] * sub_scores_g +
                    info['p_steering_mod'] * sub_scores_sm +
                    info['p_steering_ext'] * sub_scores_se
                )
                ...
                # Apply optimal coordinate blending weights (Step 43 optimal weights)
                if blend_priors:
                    optimal_weights = {
        ...
        out_path = Path(f"outputs/step43_selective_3d_routing/submission{suffix}.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 43 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ Finished: [Step 43 inference.py] Submission created successfully!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ Failed: [Step 43 inference.py] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 43 Tri-Model Inference")
    ...
    run_step43_inference(batch_size=args.batch_size, blend_priors=not args.no_blend)
