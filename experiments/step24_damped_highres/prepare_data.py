...
from step24_damped_highres.physics import make_candidates, CANDIDATES, EPS
...
def prepare_damped_highres_data():
    ...
    print(f"Preparing Step 24 Damped High-Res Data (Threshold: {TARGET_THRESHOLD}m)...")
    ...
    out_path = Path("step24_damped_highres/train_ranker_v24.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 24 Data saved to {out_path} ({len(out_df)} rows)")
    print(f"Positive samples (<= 1.0cm): {positives} ({positives/len(out_df):.2%})")

if __name__ == "__main__":
    prepare_damped_highres_data()
