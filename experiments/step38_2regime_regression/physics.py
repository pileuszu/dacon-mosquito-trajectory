...
def make_candidates(x, priors, end_idx=-1, horizon=2, regime=None):
    ...
    # 1. Regime-based Smoothing Window and Grid Selection
    if regime == "slow_straight":
        # Slow straight: uses w5 smoothing, slow specs base, dynamic lateral curve scaling
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        # Scale grid dynamically to handle sudden 39.5 deg turns
        S_grid = float(np.clip(1.0 + 0.15 * ctx_curv, 1.0, 1.8))
        
    elif regime == "fast_straight":
        # Fast straight: uses w5 smoothing, fast specs base, standard grid scaling
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        cands_base = list(CANDIDATES_FAST)
        S_grid = float(np.clip(1.0 + 0.6 * p_sacc, 1.0, 2.5))
        
    elif regime == "slow_extreme_turning":
        # Slow Extreme Turning: uses w3 smoothing to remove phase lag, fast specs base, huge grid scaling
        x_w3 = x_sliced[-3:]
        coeffs_w3 = PINV_W3_QUAD @ x_w3
        acc_smooth = 2.0 * coeffs_w3[0]
        
        # Redefine speed and tangent using W3 to align with current turn direction
        speed_w3 = np.linalg.norm(coeffs_w3[1])
        tangent = coeffs_w3[1] / (speed_w3 + EPS)
        
        # Add wider turning specs
        cands_base = list(CANDIDATES_FAST)
        S_grid = float(np.clip(1.5 + 0.1 * ctx["smooth_curv_w3"], 1.5, 3.5))
        
    elif regime == "fast_turning":
        # Fast Turning: uses w3 smoothing, fast specs base, standard fast scaling
        x_w3 = x_sliced[-3:]
        coeffs_w3 = PINV_W3_QUAD @ x_w3
        acc_smooth = 2.0 * coeffs_w3[0]
        ...
        # Saccade Termination Defense: always add straight decelerating fallback candidates
        fallback_specs = [
        ...
        cands_base = list(CANDIDATES_FAST) + fallback_specs
        S_grid = float(np.clip(1.2 + 0.6 * p_sacc, 1.2, 3.0))
        
    else:
        # Fallback to physical thresholds if regime is not specified
    ...
    # 2. Decompose smoothed acceleration
    acc_par_scalar = np.sum(acc_smooth * tangent)
    acc_par = acc_par_scalar * tangent
    acc_perp_vec = acc_smooth - acc_par
    
    # 3. Apply grid scaling
    cands_list = [replace(spec, par=spec.par * S_grid, perp=spec.perp * S_grid) for spec in cands_base]
