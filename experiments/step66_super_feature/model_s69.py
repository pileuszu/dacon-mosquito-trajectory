...
class CrossAttentionVectorFieldNet(nn.Module):
    def __init__(self, context_dim=128, time_dim=32, num_heads=8, hidden_dim=128, max_norm=0.05):
        super().__init__()
...
class SotaSpatiotemporalModel(nn.Module):
    def __init__(self, feature_dim=47, latent_dim=512, num_candidates=43, max_norm=0.05, d_mamba_in=9, dropout=0.15, num_heads=16):
        super().__init__()
        ...
        self.vector_field = CrossAttentionVectorFieldNet(context_dim=latent_dim, time_dim=32, num_heads=num_heads, hidden_dim=latent_dim, max_norm=max_norm)
