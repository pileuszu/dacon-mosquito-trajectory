...
from step23_damped_saccades.physics import make_candidates, CANDIDATES, EPS
...
def prepare_damped_physics_data():
    ...
    # 1.0cm target threshold in meters
    TARGET_THRESHOLD = 0.01
    
    print(f"Preparing Step 23 Damped Saccades Data (Threshold: {TARGET_THRESHOLD}m)...")
    ...
    for fid in tqdm(sample_ids):
        ...
        # 2. Negatives (indices not the best one and not hits)
    ...
    out_path = Path("step23_damped_saccades/train_ranker_v23.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 23 Data saved to {out_path} ({len(out_df)} rows)")
    print(f"Positive samples (<= 1.0cm): {positives} ({positives/len(out_df):.2%})")

if __name__ == "__main__":
    prepare_damped_physics_data()
