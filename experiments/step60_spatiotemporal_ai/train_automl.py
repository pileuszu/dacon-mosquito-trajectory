...
def main():
    ...
    send_discord_notification(None, f"🚀 Started: [Step 60 train_automl.py] Training 5-fold Spatiotemporal model...")
    
    try:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        data_dir = Path("step60_spatiotemporal_ai/data")
        ...
        models_dir = Path("step60_spatiotemporal_ai/models")
        ...
        for fold in range(args.folds):
            ...
            with torch.no_grad():
                _, _, _, mean_stats, std_stats = extract_features(torch.tensor(X_tr, dtype=torch.float32).to(device))
            torch.save({"mean": mean_stats.cpu(), "std": std_stats.cpu()}, models_dir / f"stats_fold_{fold}.pt")
            
            model = SotaSpatiotemporalModel(feature_dim=47, latent_dim=128, num_candidates=36, max_norm=0.08, d_mamba_in=9).to(device)
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                with torch.no_grad():
                    for Xv, yv, candv in val_loader:
                        ...
                        val_hits_1step += (torch.norm(pv_1 - yv, dim=1) <= 0.01).sum().item()
        ...
        print(f"Overall 5-Fold OOF Hit Rate@1cm (2-Step): {overall_hr_2*100:.3f}%")
        np.save(data_dir / "oof_preds_cfm_1step.npy", oof_preds_1step)
        np.save(data_dir / "oof_preds_cfm_2step.npy", oof_preds_2step)
        
        success_msg = f"✅ Finished: [Step 60] OOF 1-Step: {overall_hr_1*100:.3f}%, 2-Step: {overall_hr_2*100:.3f}%"
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [Step 60]\nError: {str(e)}\n\n{traceback.format_exc()}"
