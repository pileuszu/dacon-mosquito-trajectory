...
from step30_feature_decoupling.physics import (
    CANDIDATES_SLOW, CANDIDATES_FAST, get_damping_factors, extract_multi_scale_derivatives, PINV_W5_QUAD
)
from step30_feature_decoupling.prepare_data import extract_context_features
...
def run_step30_inference():
    try:
        send_discord_notification(URL, "🚀 [Step 30] Decoupled Features Ranker V30 Inference Started...")
        ...
        # Load Ranker V30 model
        model_path = 'step30_feature_decoupling/models/ranker_v30'
        ...
        out_path = Path("outputs/step30_feature_decoupling/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 30 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 30] Decoupled Features Ranker V30 Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 30] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_step30_inference()
