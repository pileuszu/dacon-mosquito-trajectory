...
def train_regime_model(regime_name, train_data_path, model_path, time_limit):
    try:
        send_discord_notification(URL, f"🚀 [Step 34] Regime Ranker V34 ({regime_name}) Training Started...")
        ...
        success_msg = (
            f"✅ [Step 34] Regime Ranker V34 ({regime_name}) Training Finished!\n"
        ...
        error_msg = f"❌ [Step 34] Training ERROR ({regime_name}):\n{str(e)}\n\n{traceback.format_exc()}"
...
def main():
    ...
    train_regime_model(
        regime_name="SLOW",
        train_data_path="step34_physics_explicit/train_ranker_v34_slow.csv",
        model_path="step34_physics_explicit/models/ranker_v34_slow",
        time_limit=5400
    )
    
    train_regime_model(
        regime_name="FAST",
        train_data_path="step34_physics_explicit/train_ranker_v34_fast.csv",
        model_path="step34_physics_explicit/models/ranker_v34_fast",
        time_limit=5400
    )
