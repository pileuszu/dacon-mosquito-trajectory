...
from step42_3d_out_of_plane.physics import make_candidates, extract_multi_scale_derivatives, EPS
...
def prepare_step42_data_with_params(denoise_threshold=0.98):
    print(f"\n=== Step 42: Preparing 3-Regime Datasets with Denoising Threshold = {denoise_threshold} ===")
    ...
    for fid in tqdm(sample_ids):
        ...
        if regime == "slow_moderate_turning":
            ...
            # Since 3D grid is larger, we sample slightly more negative cases to capture out-of-plane variance
    ...
    out_dir = Path("step42_3d_out_of_plane/data")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    for r_name, rows in regime_rows.items():
        df_reg = pd.DataFrame(rows)
        save_path = out_dir / f"train_ranker_v42_{r_name}_th{denoise_threshold}.csv"
...
def main():
    ...
    prepare_step42_data_with_params(denoise_threshold=0.0)
    
    # 2. Prepare the denoised dataset (threshold = 0.98) for training
    prepare_step42_data_with_params(denoise_threshold=0.98)
