...
from step22_pure_physics.physics import make_candidates, CANDIDATES, EPS
...
def prepare_pure_physics_data():
    ...
    print(f"Preparing Step 22 Pure Physics Data (Threshold: {TARGET_THRESHOLD}m)...")
    ...
    out_path = Path("step22_pure_physics/train_ranker_v22.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 22 Data saved to {out_path} ({len(out_df)} rows)")
    print(f"Positive samples (<= 1.0cm): {positives} ({positives/len(out_df):.2%})")

if __name__ == "__main__":
    prepare_pure_physics_data()
