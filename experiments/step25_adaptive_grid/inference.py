...
from step25_adaptive_grid.physics import make_candidates
from step25_adaptive_grid.prepare_data import extract_context_features
...
def run_adaptive_grid_inference_v25():
    try:
        send_discord_notification(URL, "🚀 [Step 25] Adaptive Grid Ranker V25 Inference Started...")
        
        data_dir = Path("data/open")
        test_dir = data_dir / "test"
        sample_sub = pd.read_csv(data_dir / "sample_submission.csv")
        
        # Load the Adaptive Grid Ranker V25
        ...
        print(f"Running Adaptive Grid Step 25 Inference (N={len(ids)} trajectories)...")
        ...
        out_path = Path("outputs/step25_adaptive_grid/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        sub_df.to_csv(out_path, index=False)
        
        success_msg = f"✅ [Step 25] Adaptive Grid Inference Finished Successfully!\nSaved to: {out_path}"
        send_discord_notification(URL, success_msg)
        print(success_msg)
        
    except Exception as e:
        error_msg = f"❌ [Step 25] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_adaptive_grid_inference_v25()
