...
def prepare_step46_data_with_params(denoise_threshold=0.98):
    print(f"\n=== Step 46: Preparing Hybrid Datasets with Denoising Threshold = {denoise_threshold} ===")
    ...
    out_dir = Path("step46_weighted_hybrid/data")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    for r_name, rows in regime_rows.items():
        df_reg = pd.DataFrame(rows)
        save_path = out_dir / f"train_ranker_v46_{r_name}_th{denoise_threshold}.csv"
...
def main():
    ...
    prepare_step46_data_with_params(denoise_threshold=0.0)
    
    # 2. Prepare train datasets (threshold = 0.98)
    prepare_step46_data_with_params(denoise_threshold=0.98)
