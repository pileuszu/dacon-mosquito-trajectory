...
def make_candidates(x, priors, end_idx=-1, horizon=2, regime=None):
    ...
    # 6-Regime Dynamic Scaling & Smoothing Window Routing
    if regime == "fast_straight_low":
        # Cluster 0: Medium-slow straight cruising
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        # Cluster 1: Slow turning cruising, safe using compact slow specs
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        # Cluster 2: Medium speed turning cruising
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        # Cluster 3: High speed straight cruising
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        # Cluster 4: Fast high-lateral acceleration turning (the hardest regime)
        x_w3 = x_sliced[-3:]
        coeffs_w3 = PINV_W3_QUAD @ x_w3
        ...
        # Saccade Termination Defense: add straight decelerating fallback candidates
        fallback_specs = [
        ...
        # Substantially expand grid scale to prevent target lockout in high-speed turns
        S_grid = float(np.clip(1.8 + 0.8 * p_sacc, 1.8, 3.5))
        
    elif regime == "slow_extreme_turning":
        # Cluster 5: Slow extreme curvature turning (제자리 극선회)
        x_w3 = x_sliced[-3:]
        coeffs_w3 = PINV_W3_QUAD @ x_w3
        acc_smooth = 2.0 * coeffs_w3[0]
        ...
        # Fallback if no regime specified
