...
from model import SotaCFMTrajectoryModel, extract_features, SotaFocalSoftHitLoss
...
def main():
    ...
    send_discord_notification(
        None,
        f"🚀 Started: [Step 56 train_automl.py] Training 5-fold FlowS CFM Model (Mamba + Clifford + Flow Matching)...\n"
        f"Config: epochs={args.epochs}, batch_size={args.batch_size}, lr={args.lr}, noise_scale={args.noise_scale}"
    )
    ...
    try:
        ...
        for fold in range(args.folds):
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                for Xb, yb, candb in train_loader:
                    ...
                    # 1. Forward CFM flow
                    ft, df, _, _, _ = extract_features(Xb, mean_stats, std_stats)
                    pred_v, target_v, x_0, h_context = model.forward_flow(ft, df, candb, yb, noise_scale=args.noise_scale)
                    ...
                    # 5. Semigroup consistency loss (FlowS step-consistency)
                    loss_cons = F.mse_loss(pred_1step, pred_2step)
                    
                    # 6. Combined loss
                    # Scaling loss components appropriately
                ...
                with torch.no_grad():
                    for Xv, yv, candv in val_loader:
                        ...
                        ft, df, _, _, _ = extract_features(Xv, mean_stats, std_stats)
                        
                        pv_1step = model.predict(ft, df, candv, steps=1)
                        pv_2step = model.predict(ft, df, candv, steps=2)
            ...
            fold_hr_scores_1step.append(val_hr_1step) # just tracking final epoch for 1-step as reference
        ...
        np.save("step56_flow_matching/data/oof_preds_cfm_1step.npy", oof_preds_1step)
        np.save("step56_flow_matching/data/oof_preds_cfm_2step.npy", oof_preds_2step)
        print("Saved OOF predictions for CFM.")
        
        success_msg = (
            f"✅ Finished: [Step 56 train_automl.py] SOTA FlowS CFM Model Training Complete!\n"
        ...
        error_msg = f"❌ Failed: [Step 56 train_automl.py]\nError: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
