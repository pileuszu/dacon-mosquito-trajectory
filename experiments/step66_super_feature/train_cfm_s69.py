...
from model_s69 import SotaSpatiotemporalModel
...
def main():
    ...
    parser.add_argument("--latent-dim", type=int, default=512, help="Latent embedding dimension")
    parser.add_argument("--num-heads", type=int, default=16, help="Attention heads")
    ...
    send_discord_notification(None, f"🚀 Started: [s69 Super Clifford-Mamba-CFM] latent_dim={args.latent_dim}, heads={args.num_heads}, epochs={epochs}, lambda0={lambda0}, loss_dist_weight={loss_dist_weight}, dropout={dropout}")
    ...
    try:
        ...
        models_dir_s69 = Path("step66_super_feature/models/s69")
        models_dir_s69.mkdir(parents=True, exist_ok=True)
        ...
        print(f"\n=================== Starting 5-Fold S69 Cross-Attention training ===================")
        ...
        for fold in range(folds):
            ...
            torch.save({"mean": mean_stats.cpu(), "std": std_stats.cpu()}, models_dir_s69 / f"stats_fold_{fold}.pt")
            
            # Model with larger Cross-Attention vector field (latent_dim=512, heads=16)
            ...
            for epoch in range(1, epochs + 1):
                ...
                if val_score < best_val_score:
                    ...
                    torch.save(model.state_dict(), models_dir_s69 / f"model_fold_{fold}.pt")
                    best_val_preds_1step = val_preds_1
                    best_val_preds_2step = val_preds_2
                
                # Memory efficient garbage collection
                del val_preds_1, val_preds_2, val_regs, val_dists
            ...
            model.load_state_dict(torch.load(models_dir_s69 / f"model_fold_{fold}.pt", map_location=device))
            model.eval()
            ...
            # Clear fold level variables from memory
        ...
        print(f"\n=================== JOINT-CFM S69 OOF RESULTS (4-Regime Weighted) ===================")
        ...
        np.save(models_dir_s69 / "oof_preds_cfm_1step.npy", oof_preds_1step)
        np.save(models_dir_s69 / "oof_preds_cfm_2step.npy", oof_preds_2step)
        np.save(models_dir_s69 / "test_preds_cfm_1step.npy", test_preds_1step)
        np.save(models_dir_s69 / "test_preds_cfm_2step.npy", test_preds_2step)
        
        success_msg = f"✅ Finished: [Step 69 Super Clifford-Mamba-CFM]\n" \
        ...
        error_msg = f"❌ Failed: [Step 69 Super Clifford-Mamba-CFM]\nError: {str(e)}\n\n{traceback.format_exc()}"
