...
class SotaSpatiotemporalModel(nn.Module):
    def __init__(self, feature_dim=44, latent_dim=128, num_candidates=36, max_norm=0.05, d_mamba_in=6):
        super().__init__()
    ...
    def get_anchor(self, X, features, df_cartesian, df_spherical, candidates):
        ...
        df_seq = torch.cat([df_cartesian, df_spherical], dim=-1)
...
def extract_features(X, mean_stats=None, std_stats=None):
    ...
    polaris_features = [f39, f40, f41, f42, f43, f44]
