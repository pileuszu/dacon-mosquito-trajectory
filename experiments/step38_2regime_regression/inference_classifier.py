...
def run_step38_classifier_inference(batch_size=250, model_type="l3"):
    try:
        msg = f"🚀 [Step 38 Classification] GMM 4-Regime Classification Inference Started (batch_size={batch_size}, model_type={model_type})..."
        ...
        # Load the 4 TabularPredictors from classifier models
        predictors = {}
        for regime in ["slow_straight", "fast_straight", "slow_extreme_turning", "fast_turning"]:
            model_path = Path(f"step38_2regime_regression/models/classifier_v38_{regime}")
        ...
        print(f"Running batched inference (batch_size={batch_size}, model_type={model_type}) with Classification Spatial Blending...")
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            # Predict probabilities using the routed GMM regimes
            ...
            for regime, mask in regime_masks.items():
                if np.any(mask):
                    ...
                    if model_type == "l3":
                        ...
                        proba_cat = predictor.predict_proba(pred_data_reg, model="CatBoost_BAG_L1")
                        proba_xgb = predictor.predict_proba(pred_data_reg, model="XGBoost_BAG_L1")
                        proba = 0.6 * proba_cat + 0.4 * proba_xgb
                    elif model_type == "ultra_fast":
                        proba = predictor.predict_proba(pred_data_reg, model="CatBoost_BAG_L1")
                    else:
                        proba = predictor.predict_proba(pred_data_reg)
        ...
        out_path = Path("outputs/step38_2regime_regression/submission_classifier.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 38 classification submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 38 Classification] GMM 4-Regime Classification Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 38 Classification] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 38 GMM 4-Regime Classification Inference")
    ...
    run_step38_classifier_inference(batch_size=args.batch_size, model_type=args.model_type)
