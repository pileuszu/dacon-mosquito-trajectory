...
class BiomechanicalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        xyz = self.data_cache[idx] # Read from memory cache
...
def get_dataloaders(batch_size=512):
    if METADATA_PATH.exists():
        ...
        step3_metadata = Path(__file__).parent.parent / 'step3' / 'metadata.csv'
        if step3_metadata.exists():
            df = pd.read_csv(step3_metadata)
    ...
    train_ds = BiomechanicalDataset(train_files, labels_df)
    test_ds = BiomechanicalDataset(test_files, labels_df)
    
    # ⚡ Use num_workers and pin_memory for GPU optimization
    # Note: On Windows, num_workers > 0 can sometimes be tricky with multiprocessing, 
    # but with pre-loaded data, even 2-4 should be safe and fast.
