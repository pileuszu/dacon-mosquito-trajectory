...
from step23_damped_saccades.physics import make_candidates, CANDIDATES
from step23_damped_saccades.prepare_data import extract_context_features
...
def run_damped_saccades_inference_v23():
    try:
        send_discord_notification(URL, "🚀 [Step 23] Damped Saccades Ranker V23 Inference Started...")
        
        data_dir = Path("data/open")
        test_dir = data_dir / "test"
        sample_sub = pd.read_csv(data_dir / "sample_submission.csv")
        
        # Load the Damped Saccades Ranker V23
        ...
        print(f"Running Damped Saccades Step 23 Inference (N={len(ids)} trajectories)...")
            ...
            for fid in tqdm(batch_ids, desc=f"Batch {i//batch_size + 1}"):
                ...
                for idx in range(len(cands)):
                    ...
                    row = {
                        ...
                        "spec_dmp": spec.damping,  # Key new biomechanical feature!
                        "spec_jerk": spec.jerk,
                        **ctx,
        ...
        out_path = Path("outputs/step23_damped_saccades/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        sub_df.to_csv(out_path, index=False)
        
        success_msg = f"✅ [Step 23] Damped Saccades Inference Finished Successfully!\nSaved to: {out_path}"
        send_discord_notification(URL, success_msg)
        print(success_msg)
        
    except Exception as e:
        error_msg = f"❌ [Step 23] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_damped_saccades_inference_v23()
