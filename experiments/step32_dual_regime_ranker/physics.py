...
def extract_multi_scale_derivatives(x):
    ...
    # Compute lateral acceleration and lateral jerk
...
def make_candidates(x, priors, end_idx=-1, horizon=2):
    ...
    if speed <= 0.0234:
        ...
        S_grid = 1.0  # Force unscaled for slow regime
    else:
        cands_base = list(CANDIDATES_FAST)
        S_grid = 1.0 + 0.6 * p_sacc
        
    # Scale candidate specifications
    ...
    for i, spec in enumerate(cands_list):
        ...
        # Acceleration component is generated using acc_smooth (acc_par, acc_perp_vec)
        # Note: spec.par and spec.perp are already scaled by S_grid
        cand = (p0 
