...
def get_fast_specs():
    specs = []
    # 3D Grid: par (5) * perp (7) * binormal (5) * ts (3) * damping (2) = 1050 candidates
...
def make_candidates(x, priors, end_idx=-1, horizon=2, regime=None):
    ...
    # Decompose smoothed acceleration into tangent, normal, binormal
    ...
    if regime in ["fast_extreme_turning", "slow_extreme_turning"]:
        ...
        for i, spec in enumerate(cands_list_raw):
            ...
            # Since raw specs don't have binormal in CANDIDATES_RAW_ACCEL_TURNING, binormal is 0.0
            cand = (p0 
                    + spec.d1 * (v_scale * ts * fv) * d1 
    ...
    # Compute candidate-level physical features (expanded for 3D)
    ...
    # 3D projections of candidates on normal and binormal axis
