...
def prepare_step28_data():
    ...
    print("Preparing Step 28 Advanced Dynamics Ranker Data...")
    ...
    out_path = Path("step28_anisotropic_blending/train_ranker_v28.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 28 Data saved successfully to {out_path} ({len(out_df)} rows)")
