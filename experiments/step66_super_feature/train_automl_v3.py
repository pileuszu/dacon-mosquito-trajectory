...
def main():
    send_discord_notification(None, "🚀 Started: [Step 67 train_automl_v3.py] Training 5-Fold SF-AutoML with Optimized Dynamic Grid Scaling...")
    ...
    try:
        ...
        train_df = pd.read_parquet(data_dir / "train_tabular_v3.parquet")
        train_full_df = pd.read_parquet(data_dir / "train_tabular_full_v3.parquet")
        ...
        train_candidates_hybrid = np.load(data_dir / "train_candidates_hybrid_v3.npy") # [10000, 43, 3]
        ...
        print(f"\n=================== Starting Manual {folds}-Fold Training (V3 Dynamic Grid) ===================")
        ...
        for fold in range(folds):
            ...
            if fold_model_path.exists():
                try:
                    ...
                    print(f"  Successfully loaded existing model for Fold {fold+1} from {fold_model_path}!")
                    model_loaded = True
                except Exception as e:
                    print(f"  Existing model directory {fold_model_path} could not be loaded: {e}. Re-cleaning and retraining...")
        ...
        print(f"\n=================== SF-AutoML STEP 67 (V3) OOF RESULTS ===================")
        ...
        np.save(data_dir / "oof_preds_sf_automl_v3.npy", oof_coords_1step)
        
        success_msg = f"✅ Finished: [Step 67 V3] 5-Fold training complete. 참 OOF Hit Rate@1cm: {hr_blended:.3f}% (Argmax: {hr_argmax:.3f}%)"
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [Step 67 V3]\nError: {str(e)}\n\n{traceback.format_exc()}"
