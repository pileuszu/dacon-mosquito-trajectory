...
def main():
    ...
    # Save hybrid datasets for backup / validation reference
    np.save(dest_dir / "train_cfm_preds.npy", train_cfm_preds)
    np.save(dest_dir / "test_cfm_preds.npy", test_cfm_preds)
    ...
    def get_frenet_vectors(x_data):
        ...
        B = B / (np.linalg.norm(B, axis=1)[:, None] + 1e-8)
        
        return T, N, B
    
    print("Calculating Frenet systems...")
    T_tr, N_tr, B_tr = get_frenet_vectors(train_x)
    T_te, N_te, B_te = get_frenet_vectors(test_x)
    
    p_last_tr = train_x[:, -1]
    p_last_te = test_x[:, -1]
    
    # ------------------ Build Train Tabular Dataset (43 Candidates, Sub-sampled) ------------------
    # Sub-sampling logic: best candidate (smallest distance to y) + 7 random other candidates
    print("Building training tabular DataFrame (with negative sub-sampling & 43 hybrid candidates)...")
    
    records = []
    num_train = len(train_x)
    
    # We will also save a list of the custom 43 candidates to evaluate OOF coordinates later
    ...
    for idx in range(num_train):
        ...
        # Build 7 Extra CFM-guided candidates
        delta = 0.005 # 0.5 cm
        ...
        if len(available_easy) < num_easy_to_sample:
            # Fallback to any remaining indices
        ...
        for cand_idx in selected_indices:
            ...
            # Distance from candidate to CFM prediction (Prior-decoupled spatial hint)
    ...
    train_df.to_parquet(dest_dir / "train_tabular_v2.parquet", index=False)
    np.save(dest_dir / "train_candidates_hybrid.npy", train_candidates_hybrid)
    print(f"  Saved train_tabular_v2.parquet with shape: {train_df.shape}")
    
    # ------------------ Build Full Train Tabular Dataset for True OOF Evaluation (Full 43 Candidates) ------------------
    print("Building full training tabular DataFrame for True OOF evaluation (all 43 candidates)...")
    full_records = []
    ...
    train_full_df.to_parquet(dest_dir / "train_tabular_full_v2.parquet", index=False)
    print(f"  Saved train_tabular_full_v2.parquet with shape: {train_full_df.shape}")
    
    # ------------------ Build Test Tabular Dataset (Full 43 Candidates) ------------------
    print("Building test tabular DataFrame (full 43 candidates)...")
    test_records = []
    num_test = len(test_x)
    ...
    test_df.to_parquet(dest_dir / "test_tabular_v2.parquet", index=False)
    np.save(dest_dir / "test_candidates_hybrid.npy", test_candidates_hybrid)
    print(f"  Saved test_tabular_v2.parquet with shape: {test_df.shape}")
    print("Step 67 Data preparation complete.")
