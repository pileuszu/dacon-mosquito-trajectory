...
from step42_3d_out_of_plane.physics import (
    CANDIDATES_SLOW, CANDIDATES_FAST, CANDIDATES_RAW_ACCEL_TURNING, CandidateSpec, get_damping_factors, extract_multi_scale_derivatives,
    PINV_W5_QUAD, PINV_W3_QUAD, EPS
)
from step42_3d_out_of_plane.prepare_data import extract_context_features, CLUSTER_FEATURES, REGIME_MAPPING

# Precompute multiplier matrices for vectorized 3D candidate generation
def compute_multiplier_matrix(cands_list, horizon=2):
    ...
    for i, spec in enumerate(cands_list):
        ...
        col4 = spec.binormal * (acc_scale * ts2 * fa)  # 3D Binormal coordinate column
...
# Candidate spec arrays for building test features
...
CANDS_FAST_SPECS = build_spec_dict(CANDIDATES_FAST)
CANDS_FAST_TURNING_SPECS = build_spec_dict(CANDS_FAST_TURNING)
CANDS_FAST_EXTREME_SPECS = build_spec_dict(CANDS_FAST_TURNING + CANDIDATES_RAW_ACCEL_TURNING)
CANDS_SLOW_EXTREME_SPECS = build_spec_dict(CANDIDATES_FAST + CANDIDATES_RAW_ACCEL_TURNING)
...
def make_candidates_vectorized(x, priors, end_idx=-1, horizon=2, regime=None):
    ...
    if regime == "fast_straight_low":
        ...
        spec_arr = CANDS_FAST_EXTREME_SPECS
        S_grid = float(np.clip(1.5 + 0.12 * ctx_curv, 1.5, 3.5))
        ...
        M = M_FAST.copy()
        spec_arr = CANDS_SLOW_EXTREME_SPECS
        S_grid = float(np.clip(1.5 + 0.12 * ctx_curv, 1.5, 3.5))
    ...
    # 3D Normal Plane orthonormal basis construction
    ...
    # Binormal acceleration scale vector
    ...
    M[:, 2] *= S_grid
    M[:, 3] *= S_grid
    M[:, 4] *= S_grid  # Scale binormal coordinate
    
    preds = p0 + M @ D
    
    # Vectorized Raw Acceleration Candidate generation for Extreme regimes
    if regime in ["fast_extreme_turning", "slow_extreme_turning"]:
        ...
        # Raw specs don't have binormal coordinate (binormal is 0.0)
        # So M_raw columns 4 is 0.0, which works perfectly.
    ...
    # Compute candidate features (expanded for 3D)
...
def run_step42_inference(batch_size=250, blend_priors=True):
    try:
        msg = f"🚀 Starting: [Step 42 inference.py] Batched test inference (blend_priors={blend_priors})..."
        ...
        # Load 3 Specialized AutoGluon Predictors
        print("Loading 3 Specialized AutoGluon Predictors...")
        models_dir = Path("step42_3d_out_of_plane/models")
        predictor_cruising = TabularPredictor.load(str(models_dir / "final_cruising"))
        predictor_gliding = TabularPredictor.load(str(models_dir / "final_gliding"))
        predictor_steering = TabularPredictor.load(str(models_dir / "final_steering"))
        ...
        print(f"Running batched inference with GMM Soft Routing...")
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for fid in batch_ids:
                ...
                # Predict GMM-6 cluster soft probabilities
                ...
                # Map to 3-Regime soft probabilities
                p_cruising = float(probs_6[1])
                p_gliding = float(probs_6[0] + probs_6[3])
                p_steering = float(probs_6[2] + probs_6[4] + probs_6[5])
                ...
                # Generate candidates using GMM cluster setup
            ...
            # Inject Phase 4/5 Engineered features
            ...
            # Predictions with GMM soft partitioning
            scores_c = np.zeros(len(pred_data), dtype=np.float32)
            scores_g = np.zeros(len(pred_data), dtype=np.float32)
            scores_s = np.zeros(len(pred_data), dtype=np.float32)
            
            mask_c = np.zeros(len(pred_data), dtype=bool)
            mask_g = np.zeros(len(pred_data), dtype=bool)
            mask_s = np.zeros(len(pred_data), dtype=bool)
            
            for info in batch_info:
                if info['p_cruising'] > 0.001:
                    mask_c[info['start'] : info['end']] = True
                if info['p_gliding'] > 0.001:
                    mask_g[info['start'] : info['end']] = True
                if info['p_steering'] > 0.001:
                    mask_s[info['start'] : info['end']] = True
                    
            if np.any(mask_c):
                proba_c = predictor_cruising.predict_proba(pred_data[mask_c])
                score_col_c = 1 if 1 in proba_c.columns else proba_c.columns[0]
                scores_c[mask_c] = proba_c[score_col_c].values
                
            if np.any(mask_g):
                proba_g = predictor_gliding.predict_proba(pred_data[mask_g])
                score_col_g = 1 if 1 in proba_g.columns else proba_g.columns[0]
                scores_g[mask_g] = proba_g[score_col_g].values
                
            if np.any(mask_s):
                proba_s = predictor_steering.predict_proba(pred_data[mask_s])
                score_col_s = 1 if 1 in proba_s.columns else proba_s.columns[0]
                scores_s[mask_s] = proba_s[score_col_s].values
            
            # Anisotropic Blending and Soft Routing
            for info in batch_info:
                sub_scores_c = scores_c[info['start'] : info['end']]
                sub_scores_g = scores_g[info['start'] : info['end']]
                sub_scores_s = scores_s[info['start'] : info['end']]
                
                probs = (
                    info['p_cruising'] * sub_scores_c +
                    info['p_gliding'] * sub_scores_g +
                    info['p_steering'] * sub_scores_s
                )
                ...
                # Apply optimal regime-specific coordinate blending weights (to be updated after optimize_blending run)
                if blend_priors:
                    # Default weights (will be replaced by Step 42 grid search outputs)
                    optimal_weights = {
                        "fast_straight_low": (0.70, 0.15, 0.15),
                        "slow_moderate_turning": (0.80, 0.10, 0.10),
                        "fast_moderate_turning": (1.00, 0.00, 0.00),
                        "fast_straight_high": (1.00, 0.00, 0.00),
                        "fast_extreme_turning": (1.00, 0.00, 0.00),
                        "slow_extreme_turning": (1.00, 0.00, 0.00)
                    }
        ...
        out_path = Path(f"outputs/step42_3d_out_of_plane/submission{suffix}.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 42 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ Finished: [Step 42 inference.py] Submission created successfully!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ Failed: [Step 42 inference.py] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 42 Tri-Model 3D Inference")
    ...
    run_step42_inference(batch_size=args.batch_size, blend_priors=not args.no_blend)
