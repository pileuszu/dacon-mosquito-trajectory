...
def train_regime_model(regime_name, train_data_path, model_path, time_limit):
    if Path(model_path).exists() and (Path(model_path) / "predictor.pkl").exists():
        print(f"Model for {regime_name} already exists at {model_path}. Skipping training...")
        return
    try:
        send_discord_notification(None, f"🚀 [Step 38] GMM Regime Regression Ranker ({regime_name}) Training Started...")
        ...
        # Evaluate metrics (reg_target is in meters, convert to cm in notification)
        ...
        success_msg = (
            f"✅ [Step 38] GMM Regime Regression Ranker ({regime_name}) Training Finished!\n"
            ...
            f"Best Validation MAE: **{-best_score * 100.0:.6f} cm**\n\n"
        ...
        error_msg = f"❌ [Step 38] Training ERROR ({regime_name}):\n{str(e)}\n\n{traceback.format_exc()}"
...
def main():
    ...
    train_regime_model(
        ...
        model_path="step38_2regime_regression/models/ranker_v38_slow_straight",
        time_limit=3600
    )
    ...
    train_regime_model(
        ...
        model_path="step38_2regime_regression/models/ranker_v38_fast_straight",
        time_limit=3600
    )
    ...
    train_regime_model(
        ...
        model_path="step38_2regime_regression/models/ranker_v38_slow_extreme_turning",
        time_limit=3600
    )
    ...
    train_regime_model(
        ...
        model_path="step38_2regime_regression/models/ranker_v38_fast_turning",
        time_limit=3600
    )
