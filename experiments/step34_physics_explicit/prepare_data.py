...
from step34_physics_explicit.physics import make_candidates, extract_multi_scale_derivatives, EPS
...
def prepare_step34_data():
    ...
    print("Preparing Step 34 Physics-Explicit Ranker Data...")
    ...
    for fid in tqdm(sample_ids):
        ...
        cands, cands_list, c_features = make_candidates(xyz, priors=priors, end_idx=-1, horizon=2)
        
        dists = np.linalg.norm(cands - target, axis=1)
        num_physical = len(cands_list) - 2
        
        # Determine turning state
        is_turning = (ctx["ctx_curv"] > 12.0) or (ctx["ctx_lat_accel"] > 0.0020)
        
        # Split behavior by adaptive regime routing
        if speed <= 0.0234 and not is_turning:
            ...
            for idx in selected_indices:
                ...
                row = {
                    ...
                    # Expose physical features of this candidate
            ...
            # Fast/Turning Regime: Balance candidates to prevent target dilution
    ...
    slow_path = Path("step34_physics_explicit/train_ranker_v34_slow.csv")
    slow_path.parent.mkdir(parents=True, exist_ok=True)
    slow_df.to_csv(slow_path, index=False)
    print(f"Slow cruising data saved: {slow_path} ({len(slow_df)} rows, {slow_df['target'].sum()} positives)")
    
    # Save Fast/Turning regime dataset
    fast_df = pd.DataFrame(fast_rows)
    fast_path = Path("step34_physics_explicit/train_ranker_v34_fast.csv")
    fast_df.to_csv(fast_path, index=False)
    print(f"Fast/Turning data saved: {fast_path} ({len(fast_df)} rows, {fast_df['target'].sum()} positives)")
