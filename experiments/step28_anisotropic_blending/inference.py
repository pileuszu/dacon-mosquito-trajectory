...
from step28_anisotropic_blending.physics import (
    CANDIDATES_SLOW, CANDIDATES_FAST, get_damping_factors, extract_multi_scale_derivatives
)
from step28_anisotropic_blending.prepare_data import extract_context_features
...
def make_candidates_vectorized(x, priors, end_idx=-1, horizon=2):
    ...
    return all_cands, spec_arr, speed, tangent

def run_step28_inference():
    try:
        send_discord_notification(URL, "🚀 [Step 28] Anisotropic Blending Ranker V28 Inference Started...")
        ...
        # Load Ranker V28 model
        model_path = 'step28_anisotropic_blending/models/ranker_v28'
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for info in batch_info:
                ...
                # Dynamic Anisotropic SIGMA based on speed and state probability
                sigma_tangential = np.clip(0.0035 + 0.005 * speed_m_s * p_sacc, 0.003, 0.011)
                sigma_normal = np.clip(0.0035 + 0.0015 * speed_m_s * p_sacc, 0.003, 0.006)
        ...
        out_path = Path("outputs/step28_anisotropic_blending/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 28 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 28] Anisotropic Blending Ranker V28 Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 28] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_step28_inference()
