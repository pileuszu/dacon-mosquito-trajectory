...
from step33_curvature_adaptive.physics import (
    CANDIDATES_SLOW, CANDIDATES_FAST, get_damping_factors, extract_multi_scale_derivatives, PINV_W5_QUAD
)
from step33_curvature_adaptive.prepare_data import extract_context_features
...
def make_candidates_vectorized(x, priors, end_idx=-1, horizon=2):
    ...
    return all_cands, spec_arr_dynamic, speed, tangent, is_turning, S_grid, ctx_curv

def run_step33_inference():
    try:
        send_discord_notification(URL, "🚀 [Step 33] Curvature-Adaptive Inference Started...")
        ...
        # Load Predictors
        model_path_slow = 'step33_curvature_adaptive/models/ranker_v33_slow'
        model_path_fast = 'step33_curvature_adaptive/models/ranker_v33_fast'
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for fid in batch_ids:
                ...
                cands, spec_arr, speed, tangent, is_turning, S_grid, ctx_curv = make_candidates_vectorized(xyz, priors=priors, end_idx=-1, horizon=2)
        ...
        out_path = Path("outputs/step33_curvature_adaptive/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 33 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 33] Curvature-Adaptive Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 33] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_step33_inference()
