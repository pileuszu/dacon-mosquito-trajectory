...
def evaluate_predictions(combined_df):
    ...
    idx_best = combined_df.groupby('id')['pred_dist'].idxmin()
...
def main():
    try:
        msg = "🚀 Started: [Step 44 train_automl.py] Training 3-Regime Regression specialized rankers (Th=0.8)..."
        send_discord_notification(None, msg)
        print(msg)
        
        regimes = ["cruising", "gliding", "steering"]
        
        # 1. Train the specialized regression models on denoised datasets (Th=0.8)
        for regime in regimes:
            train_path = f"step44_regression_routing/data/train_ranker_v44_{regime}_th0.8.csv"
            ...
            # Drop classification target, weight, and id columns for regression training
            ...
            print(f"\n--- Training final regression model for {regime} with best_quality and groups='fold_id' ---")
        ...
        for regime in regimes:
            train_path = f"step44_regression_routing/data/train_ranker_v44_{regime}_th0.8.csv"
            val_path = f"step44_regression_routing/data/train_ranker_v44_{regime}_th0.0.csv"
        ...
        combined_df_final.to_csv("step44_regression_routing/data/oof_predictions_final.csv", index=False)
        print("\nSaved OOF predictions to step44_regression_routing/data/oof_predictions_final.csv")
        
        overall_hit, best_cands, breakdowns = evaluate_predictions(combined_df_final)
        
        success_msg = (
            f"✅ Finished: [Step 44 train_automl.py] Success!\n"
            f"Overall Selective 3D Routing Regression Leak-Free OOF Hit@1cm: **{overall_hit:.4%}**\n"
            f"Regime Breakdown:\n"
        )
        ...
        error_msg = f"❌ Failed: [Step 44 train_automl.py] ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
