...
from model_s68 import SotaSpatiotemporalModel
...
def main():
    ...
    parser.add_argument("--latent-dim", type=int, default=256, help="Latent embedding dimension")
    ...
    parser.add_argument("--loss-dist-weight", type=float, default=300.0, help="Weight for direct L1 distance loss")
    ...
    send_discord_notification(None, f"🚀 Started: [s68 Larger Adaptive CFM] latent_dim={args.latent_dim}, heads={args.num_heads}, lambda0={lambda0}, loss_dist_weight={loss_dist_weight}, dropout={dropout}")
    ...
    try:
        ...
        models_dir_s68 = Path("step66_super_feature/models/s68")
        models_dir_s68.mkdir(parents=True, exist_ok=True)
        
        # Load datasets
        X_train = np.load(data_dir_s65 / "train_x.npy")
        y_train = np.load(data_dir_s65 / "train_y.npy")
        candidates_train = np.load(data_dir_s66 / "train_candidates_hybrid_v3.npy") # [10000, 43, 3]
        with open(data_dir_s65 / "train_ids.json", "r") as f:
            train_ids = json.load(f)
            
        test_x = np.load(data_dir_s65 / "test_x.npy")
        test_candidates = np.load(data_dir_s66 / "test_candidates_hybrid_v3.npy") # [10000, 43, 3]
        ...
        print(f"\n=================== Starting 5-Fold S68 Cross-Attention training ===================")
        ...
        for fold in range(folds):
            ...
            torch.save({"mean": mean_stats.cpu(), "std": std_stats.cpu()}, models_dir_s68 / f"stats_fold_{fold}.pt")
            
            # Model with Cross-Attention vector field (num_candidates=43)
            model = SotaSpatiotemporalModel(
                feature_dim=47, 
            ...
            for epoch in range(1, epochs + 1):
                ...
                if val_score < best_val_score:
                    ...
                    torch.save(model.state_dict(), models_dir_s68 / f"model_fold_{fold}.pt")
            ...
            model.load_state_dict(torch.load(models_dir_s68 / f"model_fold_{fold}.pt", map_location=device))
            model.eval()
        ...
        props = {0: 0.1514, 1: 0.2795, 2: 0.2343, 3: 0.3348} # We use the correct 4-regime classification mapped weights for evaluation
        # Let's map regimes_tr (3 components) to evaluation
        regimes_eval = classify_regimes_np(X_train)
        
        # Note: classify_regimes_np outputs 0, 1, 2. Let's evaluate using 4-regime weights by matching speeds & curvature:
        # We classify 4 regimes for correct evaluation
        regimes_4r, _, _, _, _ = classify_regimes_np_4r(X_train) if 'classify_regimes_np_4r' in globals() else classify_4regimes_eval(X_train)
        ...
        print(f"\n=================== JOINT-CFM S68 OOF RESULTS (4-Regime Weighted) ===================")
        ...
        np.save(models_dir_s68 / "oof_preds_cfm_1step.npy", oof_preds_1step)
        np.save(models_dir_s68 / "oof_preds_cfm_2step.npy", oof_preds_2step)
        np.save(models_dir_s68 / "test_preds_cfm_1step.npy", test_preds_1step)
        np.save(models_dir_s68 / "test_preds_cfm_2step.npy", test_preds_2step)
        
        success_msg = f"✅ Finished: [Step 68 Larger CFM]\n" \
        ...
        error_msg = f"❌ Failed: [Step 68 Larger CFM]\nError: {str(e)}\n\n{traceback.format_exc()}"
        print(error_msg, file=sys.stderr)
        send_discord_notification(None, error_msg[:1900])
        sys.exit(1)

def classify_4regimes_eval(X):
    EPS = 1e-8
    N = X.shape[0]
    last_v = (X[:, -1] - X[:, -2]) / 0.04
    speeds = np.linalg.norm(last_v, axis=1)
