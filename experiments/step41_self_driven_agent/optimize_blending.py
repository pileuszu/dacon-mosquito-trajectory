...
from step41_self_driven_agent.prepare_data import extract_context_features, REGIME_MAPPING

def search_best_weights(trajectory_data, true_targets, s4_priors, s7_priors, num_search_steps=20):
    """
    Runs a grid search over coordinate blending weights (w_m, w_4, w_7) that sum to 1.
    Evaluates Hit@1cm on the given trajectory data.
    """
    best_hit = -1.0
    best_weights = None
...
def main():
    print("=== Step 41: Optimizing Coordinate Blending Weights on Unbiased OOF Validation ===")
    data_dir = Path("data/open")
    train_dir = data_dir / "train"
    
    # 1. Load data labels and priors
    ...
    # 2. Load OOF final predictions
    print("Loading OOF final predictions...")
    oof_df = pd.read_csv("step41_self_driven_agent/data/oof_predictions_final.csv")
    
    # Sort by ID and cand_idx
    ...
    # 3. Cache inputs for speed
    true_targets = {}
    s4_priors = {}
    s7_priors = {}
    
    # Group trajectory predictions
    print("Grouping predictions by trajectory...")
    oof_grouped = {fid: gp for fid, gp in oof_df.groupby('id')}
    
    # Generate trajectories features and candidates
    trajectory_data = {}
    
    print("Generating validation candidates and running anisotropic smoothing...")
    for fid in tqdm(unique_ids):
        # Load context - optimized raw string read to speed up by 10x
        ...
        # Map predictions to matching cands by index
        cand_indices = gp['cand_idx'].values
        pred_probs = gp['pred_prob'].values
        
        probs = np.zeros(len(cands), dtype=np.float32)
        for idx, p in zip(cand_indices, pred_probs):
            if idx < len(probs):
                probs[idx] = p
        ...
        # Calculate raw error distance (distance of final_coords to target)
    ...
    # Print baseline (raw) overall performance
    overall_raw_hit = np.mean([info['raw_hit'] for info in trajectory_data.values()])
    print(f"\nBaseline (Anisotropic Smoothed only, no coordinate blending) Overall Hit@1cm: {overall_raw_hit:.4%}")
    
    # 4. Group trajectories by GMM cluster and optimize separately
    cluster_results = {}
    regime_names = {
        0: "fast_straight_low",
    ...
    for cluster_id in range(6):
        ...
        # Baseline raw hit rate for this cluster
