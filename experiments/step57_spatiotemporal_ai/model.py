...
# =====================================================================
# 4. CONTINUOUS VECTOR FIELD NETWORK WITH TANGENT SPACE PROJECTION
# =====================================================================
class TangentProjectedVectorFieldNet(nn.Module):
    def __init__(self, context_dim=64, time_dim=32, hidden_dim=128, max_norm=0.06):
        super().__init__()
    ...
    def forward(self, x_t, t, h_context):
        ...
        # Invariant Compiler: Tangent Space Projection Constraint
        # Clamps prediction vector norm within physical flight boundary limits (max_norm)
        ...
        v_projected = v_t * scale
        
        return v_projected


# =====================================================================
# 5. STEP 57 MODEL (SPHERICAL DYNAMICS + RELATION BRANCH + TANGENT SPACE)
# =====================================================================
class SotaSpatiotemporalModel(nn.Module):
    def __init__(self, feature_dim=44, latent_dim=64, num_candidates=36, max_norm=0.06):
        super().__init__()
        self.num_candidates = num_candidates
        self.latent_dim = latent_dim
        self.max_norm = max_norm
        
        # 1. Temporal Encoder (Cartesian + Spherical sequence)
        # Sequence input dimension: Cartesian df (3) + Spherical df (3) = 6
        self.mamba_encoder = NativeMambaBlock(d_model=6, d_state=16, expand=4)
        ...
        # 3. Explicit Spatial-Temporal Relational Branching (DSTM 영감)
        # Pairwise distance feature from 11 trajectory points to 36 candidate points
        ...
        # 4. Clifford Embeddings for Candidates
        ...
        # 5. Joint Attention over 36 Anchors
        self.joint_attention = nn.Sequential(
            nn.Linear(latent_dim * 3, latent_dim), # Mamba+Context (1) + Clifford (1) + Relational (1)
            nn.GELU(),
            nn.Linear(latent_dim, 1)
        )
        
        # 6. Local 3D Refinement Offset
        ...
        # 7. Tangent Projected Vector Field Net
    ...
    def get_anchor(self, X, features, df_cartesian, df_spherical, candidates):
        # X: (batch, 11, 3) Cartesian history
        # candidates: (batch, 36, 3)
        batch_size = X.shape[0]
        
        # 1. Temporal sequence encoding (Mamba with hybrid sequence)
        df_seq = torch.cat([df_cartesian, df_spherical], dim=-1) # (batch, 10, 6)
        ...
        h_joint_ctx = h_temp + h_ctx # (batch, latent_dim)
        
        # 3. Explicit Relational Branching
        # Compute pairwise distance matrix from history X to candidates
        # X: (batch, 11, 3) -> (batch, 11, 1, 3)
        # candidates: (batch, 36, 3) -> (batch, 1, 36, 3)
        X_exp = X.unsqueeze(2).expand(-1, -1, self.num_candidates, -1)
        cand_exp = candidates.unsqueeze(1).expand(-1, 11, -1, -1)
        rel_matrix = torch.norm(cand_exp - X_exp, dim=-1) # (batch, 11, 36)
        
        # Project relational distance maps
        rel_matrix_t = rel_matrix.transpose(1, 2) # (batch, 36, 11)
        h_rel = self.rel_embed(rel_matrix_t) # (batch, 36, latent_dim)
        
        # 4. Clifford multivector candidates representation
    ...
    def forward_flow(self, X, features, df_cartesian, df_spherical, candidates, y, noise_scale=0.01):
        ...
        # Add local perturbation to anchor
...
# =====================================================================
# 6. SPHERICAL DYNAMICS & PHYSICAL 44D FEATURE EXTRACTION
# =====================================================================
def extract_features(X, mean_stats=None, std_stats=None):
    batch_size = X.shape[0]
    
    # 1. Cartesian Velocity, Acceleration, Jerk
    v = (X[:, 1:] - X[:, :-1]) / 0.04  # (batch, 10, 3)
    a = (v[:, 1:] - v[:, :-1]) / 0.04  # (batch, 9, 3)
    j = (a[:, 1:] - a[:, :-1]) / 0.04  # (batch, 8, 3)
    
    speeds = torch.norm(v, dim=2)
    acc_norms = torch.norm(a, dim=2)
    jerk_norms = torch.norm(j, dim=2)
    
    # Base 38D physical features (identical to step55/step56)
    f1 = speeds[:, -1:]
    f2 = acc_norms[:, -1:]
    f3 = jerk_norms[:, -1:]
    ...
    # 2. Polaris-inspired Spherical Coordinate Transformation
    # Convert history trajectory X (batch, 11, 3) to Spherical Coordinates (rho, theta, phi)
    # Target-centered or last-point relative? We transform the global raw trajectories
    # to maintain strict geometric scale relative to origin.
    rho_seq = torch.norm(X, dim=2) # (batch, 11)
    theta_seq = torch.atan2(X[:, :, 1], X[:, :, 0]) # (batch, 11)
    phi_seq = torch.acos(torch.clamp(X[:, :, 2] / (rho_seq + BaseEPS), -1.0, 1.0)) # (batch, 11)
    
    # Compute relative changes in spherical coordinates (spherical displacement)
    d_rho = (rho_seq[:, 1:] - rho_seq[:, :-1]) / 0.04 # (batch, 10)
    d_theta = (theta_seq[:, 1:] - theta_seq[:, :-1]) / 0.04
    d_phi = (phi_seq[:, 1:] - phi_seq[:, :-1]) / 0.04
    
    # Unwrap phase wraps for angles
    # d_theta can have jumps of 2*pi, unwrap it
    d_theta = torch.remainder(d_theta + np.pi, 2 * np.pi) - np.pi
    d_phi = torch.remainder(d_phi + np.pi, 2 * np.pi) - np.pi
    
    # Stack spherical velocity representation (batch, 10, 3)
    df_spherical = torch.stack([d_rho, d_theta, d_phi], dim=-1)
    
    # Extract Spherical static features (6차원 추가 -> 총 44D)
    f39 = d_rho[:, -1:] # Last spherical radial speed
    f40 = d_theta[:, -1:] # Last angular azimuth speed
    f41 = d_phi[:, -1:] # Last angular polar speed
