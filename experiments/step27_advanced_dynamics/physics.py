...
def extract_multi_scale_derivatives(x):
    ...
    # 2. W3 Quadratic Fit (Last 3 points: -2, -1, 0)
    # Analytical coefficients:
    # a_w3 = 0.5*x[-3] - x[-2] + 0.5*x[-1] (since t = -2, -1, 0 is used for indexing relative to end)
    # Let's fit indices t = [-2, -1, 0]:
    # x_sub_w3 = x[-3:] -> indices -2, -1, 0
    # a_w3 = 0.5*x[-3] - x[-2] + 0.5*x[-1]
    # b_w3 = 0.5*x[-3] - 2*x[-2] + 1.5*x[-1]
    x_w3 = x[-3:]
