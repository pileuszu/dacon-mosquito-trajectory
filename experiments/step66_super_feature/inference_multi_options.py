...
def main():
    send_discord_notification(None, "🚀 Started: [Step 66 inference_multi_options.py] Running Multi-Option Ensemble Damping Inference...")
    ...
    try:
        ...
        # Powell-Optimized Blend Weights
        w_cr = [0.5065, 0.0510, 0.0, 0.0, 0.0510, 0.2386, 0.1528, 0.0]
        w_gl = [0.2964, 0.0, 0.3518, 0.2513, 0.0, 0.1005, 0.0, 0.0]
        w_st = [0.5479, 0.1999, 0.0498, 0.1008, 0.0, 0.0, 0.1015, 0.0]
        ...
        print("Reconstructing Powell blended test coordinates...")
        ...
        # Candidate spread for easy/hard routing
        ...
        s65_guidance_te = s65_test_1step
        
        options = [
            {"name": "pure_damped", "th": 0.80, "shrink": 0.70, "gamma": 0.0},
            {"name": "moderate_guided", "th": 0.80, "shrink": 0.70, "gamma": 0.50}
        ]
        ...
        for opt in options:
            ...
            print(f"\nProcessing option: {name} (th={th}, shrink={shrink}, gamma={gamma})")
            ...
            for idx in range(len(test_x)):
                if prob_miss_te[idx] > th and pred_disp_norm_te[idx] > 0.015:
                    ...
                    corrected_test[idx] = (1.0 - gamma) * damped_coord + gamma * s65_guidance_te[idx]
                    damped_count += 1
            ...
            for idx in range(len(test_ids)):
                if easy_mask[idx]:
                    # Snap raw blended coordinate (Consensus) to the nearest hybrid candidate
            ...
            sub_path = out_dir / f"submission_final_{name}.csv"
            print(f"Writing submission to {sub_path}...")
            ...
            disp_stats = f"Option: {name} | Displacement stats (cm) -> Mean: {diffs.mean()*100:.3f}, Max: {diffs.max()*100:.3f}, Std: {diffs.std()*100:.3f}"
            print(disp_stats)
            
            success_msg = f"✅ Option [{name}] generated. Mean: {diffs.mean()*100:.3f}cm, Damped: {damped_count}, Easy Snapped: {easy_snapped_count}"
            send_discord_notification(None, success_msg)
            
    except Exception as e:
        error_msg = f"❌ Failed: [Step 66 inference_multi_options.py] Multi-option inference failed.\nError: {str(e)}\n\n{traceback.format_exc()}"
