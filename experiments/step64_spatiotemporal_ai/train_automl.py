...
def main():
    ...
    send_discord_notification(None, f"🚀 Started: [Step 64 train_automl.py] Training 3-Regime Split-CFM models with Frenet-Flow loss...")
    ...
    try:
        ...
        lambdas = [10.0, 5.0, 1.0] # [Cruising, Gliding, Steering] Frenet-Flow lambda weight
        
        # We save global normalization stats for test reconstruction per regime & fold
        for regime in range(3):
            regime_mask = regimes_tr == regime
            regime_name = regime_names[regime]
            lambda_physics = lambdas[regime]
            
            print(f"\n=================== Training Regime: {regime_name.upper()} (N={np.sum(regime_mask)}) ===================")
            
            X_reg, y_reg, cand_reg, fold_reg = X_train[regime_mask], y_train[regime_mask], candidates_train[regime_mask], fold_ids[regime_mask]
            
            for fold in range(args.folds):
                train_mask = fold_reg != fold
                val_mask = fold_reg == fold
                
                X_tr, y_tr, cand_tr = X_reg[train_mask], y_reg[train_mask], cand_reg[train_mask]
                X_val, y_val, cand_val = X_reg[val_mask], y_reg[val_mask], cand_reg[val_mask]
                
                train_loader = DataLoader(MosquitoCandidatesDataset(X_tr, y_tr, cand_tr), batch_size=args.batch_size, shuffle=True)
                val_loader = DataLoader(MosquitoCandidatesDataset(X_val, y_val, cand_val), batch_size=256, shuffle=False)
                
                # Extract mean and std stats for features
                with torch.no_grad():
                    _, _, _, mean_stats, std_stats, _ = extract_features(torch.tensor(X_tr, dtype=torch.float32).to(device))
                torch.save({"mean": mean_stats.cpu(), "std": std_stats.cpu()}, models_dir / f"stats_{regime_name}_fold_{fold}.pt")
                ...
                for epoch in range(1, args.epochs + 1):
                    ...
                    for Xb, yb, candb in train_loader:
                        ...
                        optimizer.zero_grad()
                        ft, df_c, df_s, _, _, (t_v, n_v, b_v) = extract_features(Xb, mean_stats, std_stats)
                        ...
                        # Frenet Transverse flow velocity Loss (L_physics)
                        v_N = torch.sum(pred_v * n_v, dim=1)
                        v_B = torch.sum(pred_v * b_v, dim=1)
                        loss_frenet = (v_N**2 + v_B**2).mean()
                        
                        loss_flow = loss_cfm + lambda_physics * loss_frenet
                    ...
                    with torch.no_grad():
                        for Xv, yv, candv in val_loader:
                            ...
                            ft, df_c, df_s, _, _, _ = extract_features(Xv, mean_stats, std_stats)
                            pv_1 = model.predict(Xv, ft, df_c, df_s, candv, steps=1)
                            pv_2 = model.predict(Xv, ft, df_c, df_s, candv, steps=2)
                    ...
                    if val_hr_2 >= best_val_hr_2step:
                        ...
                        torch.save(model.state_dict(), models_dir / f"model_{regime_name}_fold_{fold}.pt")
                ...
                # Map OOF predictions back to correct indices
                # train_y index alignment
                regime_indices = np.where(regime_mask)[0]
                val_indices = regime_indices[val_mask]
                oof_preds_1step[val_indices] = best_val_preds_1step
                oof_preds_2step[val_indices] = best_val_preds_2step
                
                print(f"[{regime_name.upper()}] Fold {fold+1} Finished. Best Hit@1cm: {best_val_hr_2step*100:.3f}%")
        ...
        success_msg = f"✅ Finished: [Step 64] OOF 1-Step: {overall_hr_1*100:.3f}%, 2-Step: {overall_hr_2*100:.3f}%"
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [Step 64]\nError: {str(e)}\n\n{traceback.format_exc()}"
