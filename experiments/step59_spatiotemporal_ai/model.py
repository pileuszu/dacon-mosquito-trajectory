...
class SotaSpatiotemporalModel(nn.Module):
    def __init__(self, feature_dim=47, latent_dim=128, num_candidates=36, max_norm=0.08, d_mamba_in=9):
        super().__init__()
...
def extract_features(X, mean_stats=None, std_stats=None):
    ...
    # Compute spherical acceleration
    a_spherical = df_spherical[:, 1:] - df_spherical[:, :-1]
    f45 = a_spherical[:, -1, 0:1] # radial acceleration
    f46 = a_spherical[:, -1, 1:2] # angular azimuth acc
    f47 = a_spherical[:, -1, 2:3] # angular polar acc
