...
def prepare_tabular_data(limit=10000):
    ...
    print(f"Preparing Step 10 tabular data for {len(sample_ids)} samples...")
    for fid in tqdm(sample_ids):
        ...
        for idx in selected_indices:
            ...
            row = {
                ...
                "spec_perp": spec.perp,
                "spec_ts": spec.ts,
                "dist_to_s7": dists_to_s7[idx], # KEY ENSEMBLE FEATURE
                **ctx,
                "dist_to_p0": np.linalg.norm(cands[idx] - xyz[-1]),
                "target": 1 if dist <= 0.01 else 0,
                "reg_target": dist
            }
            rows.append(row)
            
    out_df = pd.DataFrame(rows)
    out_path = Path("step10/train_ranker.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    print(f"Step 10 tabular data saved to {out_path} ({len(out_df)} rows)")
