...
def inference():
    ...
    model_path = 'step5_best_model.pth'
    ...
    print(f"Starting Multi-modal inference on {len(test_files)} samples...")
    with torch.no_grad():
        for fpath in tqdm(test_files):
            ...
            # Predict (Model handles coordinate reconstruction internally for the best mode)
            final_pred, _, _, _ = model(seq_t, cv_prior_t, last_pos_t, rot_mat_t)
    ...
    res_df.to_csv('outputs/step5/submission.csv', index=False)
    print(f"Submission saved to outputs/step5/submission.csv")
