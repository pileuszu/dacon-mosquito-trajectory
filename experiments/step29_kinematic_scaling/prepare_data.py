...
def prepare_step29_data():
    ...
    print("Preparing Step 29 Advanced Dynamics Ranker Data...")
    ...
    out_path = Path("step29_kinematic_scaling/train_ranker_v29.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_df.to_csv(out_path, index=False)
    
    positives = out_df['target'].sum()
    print(f"Step 29 Data saved successfully to {out_path} ({len(out_df)} rows)")
