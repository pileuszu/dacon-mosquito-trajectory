...
def run_step38_raw_inference(batch_size=250, model_type="l3"):
    try:
        msg = f"🚀 [Step 38 Raw] GMM 4-Regime Regression RAW Inference Started (batch_size={batch_size}, model_type={model_type})..."
        ...
        print(f"Running batched inference (batch_size={batch_size}, model_type={model_type}) with Raw Minimum Distance Selection...")
        ...
        out_path = Path("outputs/step38_2regime_regression/submission_raw.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 38 raw submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 38 Raw] GMM 4-Regime Regression RAW Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 38 Raw] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 38 GMM 4-Regime Regression Raw Inference")
    ...
    run_step38_raw_inference(batch_size=args.batch_size, model_type=args.model_type)
