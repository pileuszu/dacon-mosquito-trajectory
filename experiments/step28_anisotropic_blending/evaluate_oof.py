...
from step28_anisotropic_blending.physics import make_candidates, extract_multi_scale_derivatives, EPS

def evaluate_step28_oof():
    print("Loading Step 28 training data and predictions...")
    train_data_path = 'step28_anisotropic_blending/train_ranker_v28.csv'
    ...
    # We can determine if it is slow or fast based on 'ctx_speed'
    ...
    for fid in tqdm(unique_ids):
        ...
        sub_df = df[df['id'] == fid]
        # In train_ranker_v28.csv, we filtered/balanced candidates for fast trajectories.
        # Let's map OOF probabilities to the reconstructed candidates.
        # Default probability is 0 for candidates not in training subset.
        probs = np.zeros(len(cands), dtype=np.float32)
        for _, r in sub_df.iterrows():
            probs[int(r['cand_idx'])] = r['oof_prob']
