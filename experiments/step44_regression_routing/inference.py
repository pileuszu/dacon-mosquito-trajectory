...
from step44_regression_routing.physics import (
...
from step44_regression_routing.prepare_data import extract_context_features, CLUSTER_FEATURES, REGIME_MAPPING
...
def run_step44_inference(batch_size=250, blend_priors=True):
    try:
        msg = f"🚀 Starting: [Step 44 inference.py] Batched test inference (blend_priors={blend_priors})..."
        ...
        # Load 3 Specialized Regression AutoGluon Predictors
        print("Loading 3 Specialized AutoGluon Predictors...")
        models_dir = Path("step44_regression_routing/models")
        ...
        print(f"Running batched inference with GMM Soft Routing in Regression Mode...")
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            # Predictions (MAE Regression predicts distance)
            dists_c = np.zeros(len(pred_data), dtype=np.float32)
            dists_g = np.zeros(len(pred_data), dtype=np.float32)
            dists_s = np.zeros(len(pred_data), dtype=np.float32)
            ...
            if np.any(mask_c):
                dists_c[mask_c] = predictor_cruising.predict(pred_data[mask_c]).values
                
            if np.any(mask_g):
                dists_g[mask_g] = predictor_gliding.predict(pred_data[mask_g]).values
                
            if np.any(mask_s):
                dists_s[mask_s] = predictor_steering.predict(pred_data[mask_s]).values
            
            # Convert Predicted Distances to scores: score = exp(-dist / 0.015)
            scores_c = np.exp(-dists_c / 0.015)
            scores_g = np.exp(-dists_g / 0.015)
            scores_s = np.exp(-dists_s / 0.015)
            ...
            for info in batch_info:
                ...
                # Apply optimal coordinate blending weights (Paste optimal weights here from optimize_blending)
                if blend_priors:
                    # Default/Step 43 weights as placeholder; will be updated post optimize_blending
                    optimal_weights = {
        ...
        out_path = Path(f"outputs/step44_regression_routing/submission{suffix}.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 44 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ Finished: [Step 44 inference.py] Submission created successfully!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ Failed: [Step 44 inference.py] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 44 Tri-Model Inference")
    ...
    run_step44_inference(batch_size=args.batch_size, blend_priors=not args.no_blend)
