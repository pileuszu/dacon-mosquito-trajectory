...
class SotaSpatiotemporalModel(nn.Module):
    ...
    def get_anchor(self, X, features, df_cartesian, df_spherical, candidates, regime_id):
        ...
        # Dense projection of one-hot regime embedding
        h_reg = self.regime_proj(self.regime_embed(regime_id))
        
        # Spatiotemporal contextual fusion (incorporating regime condition)
        h_joint_ctx = h_temp + h_ctx + h_reg
...
def extract_features(X, mean_stats=None, std_stats=None):
    ...
    # Raw curvature (last frame) f18
    raw_curvature = torch.norm(cross_prod, dim=1, keepdim=True) / (torch.norm(last_v, dim=1, keepdim=True) ** 3 + BaseEPS)
    f18 = raw_curvature
    ...
    t_vec = last_v / (torch.norm(last_v, dim=1, keepdim=True) + BaseEPS)
    acc_perp = last_a - torch.sum(last_a * t_vec, dim=1, keepdim=True) * t_vec
    n_norm = torch.norm(acc_perp, dim=1, keepdim=True)
    n_vec = acc_perp / (n_norm + BaseEPS)
    fallback = torch.zeros_like(n_vec)
    axis = torch.argmin(torch.abs(t_vec), dim=1)
    fallback[torch.arange(batch_size), axis] = 1.0
    fallback = fallback - torch.sum(fallback * t_vec, dim=1, keepdim=True) * t_vec
    fallback = fallback / (torch.norm(fallback, dim=1, keepdim=True) + BaseEPS)
    n_vec = torch.where(n_norm > 1e-6, n_vec, fallback)
    b_vec = torch.cross(t_vec, n_vec, dim=1)
    b_vec = b_vec / (torch.norm(b_vec, dim=1, keepdim=True) + BaseEPS)
    f29 = torch.sum(last_j * t_vec, dim=1, keepdim=True)
    f30 = torch.sum(last_j * n_vec, dim=1, keepdim=True)
    f31 = torch.sum(last_j * b_vec, dim=1, keepdim=True)
    ...
    return features_scaled, df_cartesian, df_spherical, mean_stats, std_stats, (t_vec, n_vec, b_vec), raw_curvature.squeeze(-1)

class SotaFocalSoftHitLoss(nn.Module):
    def __init__(self, delta=0.001026, alpha=200.0, target_dist=0.01):
        super().__init__()
        self.delta = delta
        self.alpha = alpha
