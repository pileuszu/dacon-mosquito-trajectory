...
from step37_turning_refinement.physics import (
...
def run_step37_inference(batch_size=250, model_type="l3"):
    try:
        ...
        # Load the 4 TabularPredictors from step37
        predictors = {}
        for regime in ["slow_straight", "fast_straight", "slow_extreme_turning", "fast_turning"]:
            model_path = Path(f"step37_turning_refinement/models/ranker_v37_{regime}")
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for info in batch_info:
                ...
                # We use a scale of 0.015 cm (1.5 cm) as the normalizer.
                probs = np.exp(- pred_dists / 0.015)
        ...
        out_path = Path("outputs/step37_turning_refinement/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 37 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 37] GMM 4-Regime Regression Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 37 GMM 4-Regime Regression Inference")
    ...
    run_step37_inference(batch_size=args.batch_size, model_type=args.model_type)
