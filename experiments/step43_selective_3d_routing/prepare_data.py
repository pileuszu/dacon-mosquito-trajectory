...
from step43_selective_3d_routing.physics import make_candidates, extract_multi_scale_derivatives, EPS
...
def get_quad_regime(gmm_idx):
    if gmm_idx == 1:
        return "cruising"
    elif gmm_idx in [0, 3]:
        return "gliding"
    elif gmm_idx in [2, 5]:
        return "steering_moderate"
    else:
        return "steering_extreme"
...
def prepare_step43_data_with_params(denoise_threshold=0.8):
    print(f"\n=== Step 43: Preparing 4-Regime Datasets with Denoising Threshold = {denoise_threshold} ===")
    ...
    traj_quad_regimes = {traj_ids[i]: get_quad_regime(clusters[i]) for i in range(len(traj_ids))}
    ...
    for fid in tqdm(sample_ids):
        ...
        quad_regime = traj_quad_regimes[fid]
        ...
        if regime == "slow_moderate_turning":
            ...
            # Since fast grids are large, we sample negative cases to balance positive targets
        ...
        for idx in selected_indices:
            ...
            regime_rows[quad_regime].append(row)
            
    # Save the pooled datasets
    out_dir = Path("step43_selective_3d_routing/data")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    for r_name, rows in regime_rows.items():
        df_reg = pd.DataFrame(rows)
        save_path = out_dir / f"train_ranker_v43_{r_name}_th{denoise_threshold}.csv"
...
def main():
    # 1. Prepare the full dataset (threshold = 0.0) for validation evaluation
    prepare_step43_data_with_params(denoise_threshold=0.0)
    
    # 2. Prepare the denoised dataset (threshold = 0.8) for training
    prepare_step43_data_with_params(denoise_threshold=0.8)
