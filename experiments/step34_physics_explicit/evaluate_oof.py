...
def evaluate_step34_oof():
    print("Loading Step 34 training data and predictions...")
    df_slow = pd.read_csv('step34_physics_explicit/train_ranker_v34_slow.csv')
    df_fast = pd.read_csv('step34_physics_explicit/train_ranker_v34_fast.csv')
    
    print("Loading AutoGluon Predictors...")
    predictor_slow = TabularPredictor.load('step34_physics_explicit/models/ranker_v34_slow')
    predictor_fast = TabularPredictor.load('step34_physics_explicit/models/ranker_v34_fast')
    ...
    for fid in tqdm(unique_ids):
        ...
        cands, specs, c_features = make_candidates(xyz, priors=priors, end_idx=-1, horizon=2)
