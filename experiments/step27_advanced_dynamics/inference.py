...
from step27_advanced_dynamics.physics import (
    CANDIDATES_SLOW, CANDIDATES_FAST, get_damping_factors, extract_multi_scale_derivatives
)
from step27_advanced_dynamics.prepare_data import extract_context_features
...
def run_step27_inference():
    try:
        send_discord_notification(URL, "🚀 [Step 27] Advanced Dynamics Ranker V27 Inference Started...")
        ...
        # Dynamic Gaussian Blending Bandwidth (Base = 3.5mm, scaling with speed)
        # We will compute it dynamically per trajectory
        
        batch_size = 500
        print(f"Running batched inference (batch_size={batch_size}) with Dynamic Gaussian Blending Bandwidth...")
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            # Post-process each trajectory in the batch using Dynamic Spatial Probability Blending
            for info in batch_info:
                probs = batch_scores[info['start'] : info['end']]
                cands = info['cands']
                
                # Dynamic SIGMA based on speed: base 3.5mm, scaling up to 10.5mm at 0.15 m/s
                sigma = np.clip(0.0035 + 0.05 * info['speed'], 0.003, 0.011)
        ...
        out_path = Path("outputs/step27_advanced_dynamics/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 27 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 27] Advanced Dynamics Ranker V27 Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 27] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_step27_inference()
