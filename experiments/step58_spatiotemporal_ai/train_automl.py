...
def main():
    ...
    send_discord_notification(None, f"🚀 Started: [Step 58 train_automl.py] Training 5-fold Spatiotemporal model...")
    
    try:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        data_dir = Path("step58_spatiotemporal_ai/data")
        ...
        models_dir = Path("step58_spatiotemporal_ai/models")
        ...
        for fold in range(args.folds):
            ...
            model = SotaSpatiotemporalModel(feature_dim=44, latent_dim=128, num_candidates=36, max_norm=0.09, d_mamba_in=6).to(device)
        ...
        success_msg = f"✅ Finished: [Step 58] OOF 1-Step: {overall_hr_1*100:.3f}%, 2-Step: {overall_hr_2*100:.3f}%"
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [Step 58]\nError: {str(e)}\n\n{traceback.format_exc()}"
