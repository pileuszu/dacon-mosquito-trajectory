...
from step35_four_regime.physics import (
    CANDIDATES_SLOW, CANDIDATES_FAST, get_damping_factors, extract_multi_scale_derivatives, PINV_W5_QUAD, EPS
)
from step35_four_regime.prepare_data import extract_context_features, CLUSTER_FEATURES
...
def run_step35_inference():
    try:
        send_discord_notification(URL, "🚀 [Step 35] GMM Regime-Specialized Inference Started...")
        ...
        out_path = Path("outputs/step35_four_regime/submission.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 35 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 35] GMM Regime-Specialized Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 35] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
        send_discord_notification(URL, error_msg)
        print(error_msg)
        raise e

if __name__ == "__main__":
    run_step35_inference()
