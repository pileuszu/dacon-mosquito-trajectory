...
def main():
    ...
    parser.add_argument("--lambda0", type=float, default=25.0, help="Base Frenet-Flow penalty weight")
    ...
    send_discord_notification(None, f"🚀 Started: [s67 Steering Expert CFM] latent_dim={args.latent_dim}, heads={args.num_heads}, lambda0={lambda0}, loss_dist_weight={loss_dist_weight}, dropout={dropout}")
    ...
    try:
        ...
        models_dir_steering.mkdir(parents=True, exist_ok=True)
        ...
        # Filter for Steering (Regime 1 or Regime 3)
        is_steering_tr = (regimes_tr_all == 1) | (regimes_tr_all == 3)
        is_steering_te = (regimes_te_all == 1) | (regimes_te_all == 3)
        
        oof_preds_1step = np.zeros_like(y_train_all)
        oof_preds_2step = np.zeros_like(y_train_all)
        
        # Initialize OOF predictions with baseline predictions or copycat from last pos for non-steering
        ...
        print(f"\n=================== Starting 5-Fold Steering Expert CFM training (N_steer={np.sum(is_steering_tr)}) ===================")
        
        for fold in range(folds):
            print(f"\n--- Fold {fold+1} / {folds} ---")
            
            # Split train / val on the entire dataset first, then filter for steering
            train_mask_all = fold_ids_all != fold
            val_mask_all = fold_ids_all == fold
            
            train_mask = train_mask_all & is_steering_tr
            val_mask = val_mask_all & is_steering_tr
            ...
            # Map regimes to 0, 1, 2 for the SotaSpatiotemporalModel (it expects range 0..2)
            # We can map: regime 1 -> 1, regime 3 -> 2
            reg_tr_mapped = np.where(reg_tr == 1, 1, 2)
            reg_val_mapped = np.where(reg_val == 1, 1, 2)
            ...
            torch.save({"mean": mean_stats.cpu(), "std": std_stats.cpu()}, models_dir_steering / f"stats_fold_{fold}.pt")
            ...
            for epoch in range(1, epochs + 1):
                ...
                # Choose checkpoint based on Hit Rate (directly!)
                val_score = -np.mean(val_dists <= 0.01) * 100.0
                
                if val_score < best_val_score:
                    best_val_score = val_score
                    torch.save(model.state_dict(), models_dir_steering / f"model_fold_{fold}.pt")
            ...
            test_reg_mapped = np.where(regimes_te_all == 1, 1, 2)
        ...
        np.save(models_dir_steering / "oof_preds_cfm_1step.npy", oof_preds_1step)
        np.save(models_dir_steering / "oof_preds_cfm_2step.npy", oof_preds_2step)
        np.save(models_dir_steering / "test_preds_cfm_1step.npy", test_preds_1step)
        np.save(models_dir_steering / "test_preds_cfm_2step.npy", test_preds_2step)
        
        # Calculate OOF Hit Rate for Steering samples
        steer_oof_dists = np.linalg.norm(oof_preds_2step[is_steering_tr] - y_train_all[is_steering_tr], axis=1)
        steer_hr = np.mean(steer_oof_dists <= 0.01) * 100.0
        
        success_msg = f"🏆 Finished training Steering Expert CFM model!\n" \
                      f"Steering OOF Hit Rate @ 1cm: {steer_hr:.4f}%"
        print(success_msg)
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [s67 Steering Expert CFM]\nError: {str(e)}\n\n{traceback.format_exc()}"
