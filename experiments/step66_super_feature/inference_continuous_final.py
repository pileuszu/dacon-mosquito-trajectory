...
def main():
    send_discord_notification(None, "🚀 Started: [Step 66 inference_continuous_final.py] Running Pure Continuous Ensemble Inference...")
    ...
    try:
        ...
        options = [
            {"name": "continuous_pure", "th": 0.80, "shrink": 0.93, "gamma": 0.0},
            {"name": "continuous_moderate", "th": 0.80, "shrink": 0.80, "gamma": 0.30}
        ]
        ...
        for opt in options:
            ...
            for idx in range(len(test_x)):
                if prob_miss_te[idx] > th and pred_disp_norm_te[idx] > 0.015:
                    ...
                    submission_coords[idx] = (1.0 - gamma) * damped_coord + gamma * s65_guidance_te[idx]
                    damped_count += 1
            
            sub_path = out_dir / f"submission_{name}.csv"
            print(f"Writing pure continuous submission to {sub_path}...")
            ...
            success_msg = f"✅ Option [{name}] generated. Mean: {diffs.mean()*100:.3f}cm, Damped: {damped_count}"
            send_discord_notification(None, success_msg)
            
    except Exception as e:
        error_msg = f"❌ Failed: [Step 66 inference_continuous_final.py] Continuous inference failed.\nError: {str(e)}\n\n{traceback.format_exc()}"
