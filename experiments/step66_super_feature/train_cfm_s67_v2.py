...
def main():
    ...
    send_discord_notification(None, f"🚀 Started: [s67 v2 Adaptive CFM] latent_dim={args.latent_dim}, heads={args.num_heads}, lambda0={lambda0}, loss_dist_weight={loss_dist_weight}, dropout={dropout}")
    ...
    try:
        ...
        models_dir_s67_v2 = Path("step66_super_feature/models/s67_v2")
        models_dir_s67_v2.mkdir(parents=True, exist_ok=True)
        ...
        print(f"\n=================== Starting 5-Fold S67 V2 Cross-Attention training ===================")
        ...
        for fold in range(folds):
            ...
            torch.save({"mean": mean_stats.cpu(), "std": std_stats.cpu()}, models_dir_s67_v2 / f"stats_fold_{fold}.pt")
            ...
            for epoch in range(1, epochs + 1):
                ...
                if val_score < best_val_score:
                    ...
                    torch.save(model.state_dict(), models_dir_s67_v2 / f"model_fold_{fold}.pt")
            ...
            model.load_state_dict(torch.load(models_dir_s67_v2 / f"model_fold_{fold}.pt", map_location=device))
            model.eval()
        ...
        print(f"\n=================== JOINT-CFM S67 V2 OOF RESULTS (Test-Proportion Weighted) ===================")
        ...
        np.save(models_dir_s67_v2 / "oof_preds_cfm_1step.npy", oof_preds_1step)
        np.save(models_dir_s67_v2 / "oof_preds_cfm_2step.npy", oof_preds_2step)
        np.save(models_dir_s67_v2 / "test_preds_cfm_1step.npy", test_preds_1step)
        np.save(models_dir_s67_v2 / "test_preds_cfm_2step.npy", test_preds_2step)
        
        success_msg = f"✅ Finished: [Step 67 Adaptive CFM (V2)]\n" \
        ...
        error_msg = f"❌ Failed: [Step 67 Adaptive CFM (V2)]\nError: {str(e)}\n\n{traceback.format_exc()}"
