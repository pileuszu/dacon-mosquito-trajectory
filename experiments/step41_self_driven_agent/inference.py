...
from step41_self_driven_agent.physics import (
...
from step41_self_driven_agent.prepare_data import extract_context_features, CLUSTER_FEATURES, REGIME_MAPPING
...
def make_candidates_vectorized(x, priors, end_idx=-1, horizon=2, regime=None):
    ...
    if regime == "fast_straight_low":
        ...
        M = M_FAST.copy()
        spec_arr = CANDS_FAST_SPECS
        S_grid = float(np.clip(1.2 + 0.5 * p_sacc, 1.2, 2.2))
        
    elif regime == "fast_extreme_turning":
        # w3 window quadratic fit to prevent lag
        acc_smooth = acc
        if len(x_sliced) >= 3:
            x_w3 = x_sliced[-3:]
    ...
    acc_par = np.sum(acc_smooth * tangent) * tangent
    ...
    if regime in ["fast_extreme_turning", "slow_extreme_turning"]:
        ...
        D_raw = np.vstack([d1, d2, acc_par_raw, acc_perp_vec_raw, jerk])
...
def run_step41_inference(batch_size=250, blend_priors=True):
    try:
        msg = f"🚀 [Step 41] Tri-Model Inference Started (batch_size={batch_size}, blend_priors={blend_priors})..."
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            for fid in batch_ids:
                ...
                cands, spec_arr, speed, tangent, is_turning, S_grid, ctx_curv, c_features = make_candidates_vectorized(
            ...
            # Inject Phase 4 Engineered features!
            ...
            for info in batch_info:
                ...
                # Apply optimal regime-specific coordinate blending weights
                if blend_priors:
                    optimal_weights = {
                        "fast_straight_low": (0.80, 0.15, 0.05),
        ...
        out_path = Path(f"outputs/step41_self_driven_agent/submission{suffix}.csv")
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_df.to_csv(out_path, index=False)
        print(f"Step 41 submission saved to {out_path}")
        ...
        success_msg = (
            f"✅ [Step 41] Stacking-Ensemble Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
        ...
        error_msg = f"❌ [Step 41] Inference ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 41 Tri-Model Soft-Blending Inference")
    ...
    run_step41_inference(batch_size=args.batch_size, blend_priors=not args.no_blend)
