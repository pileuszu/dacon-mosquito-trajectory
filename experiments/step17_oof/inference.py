...
def run_clean_inference_v17():
    data_dir = Path("data/open")
    test_dir = data_dir / "test"
    sample_sub = pd.read_csv(data_dir / "sample_submission.csv")
    
    # Use standard Step 7 & 4 priors for Test (These are honest by default as they haven't seen Test targets)
    ...
    # Load HONEST Ranker V17
    model_path = 'step17_oof/models/ranker_v17'
    ...
    print(f"Running Clean Step 17 Inference...")
    ...
    out_path = Path("outputs/step17/submission.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    sub_df.to_csv(out_path, index=False)
    print(f"CLEAN Submission saved to {out_path}")

if __name__ == "__main__":
    run_clean_inference_v17()
