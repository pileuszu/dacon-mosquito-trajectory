...
def prepare_step44_data_with_params(denoise_threshold=0.8):
    print(f"\n=== Step 44: Preparing 3-Regime Datasets with Denoising Threshold = {denoise_threshold} ===")
    ...
    out_dir = Path("step44_regression_routing/data")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    for r_name, rows in regime_rows.items():
        df_reg = pd.DataFrame(rows)
        save_path = out_dir / f"train_ranker_v44_{r_name}_th{denoise_threshold}.csv"
...
def main():
    ...
    prepare_step44_data_with_params(denoise_threshold=0.0)
    
    # 2. Prepare the denoised dataset (threshold = 0.8) for training
    prepare_step44_data_with_params(denoise_threshold=0.8)
