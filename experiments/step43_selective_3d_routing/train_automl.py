...
def main():
    try:
        msg = "🚀 Started: [Step 43 train_automl.py] Training 4-Regime Selective 3D Grid specialized rankers (Th=0.8)..."
        send_discord_notification(None, msg)
        print(msg)
        
        regimes = ["cruising", "gliding", "steering_moderate", "steering_extreme"]
        
        # 1. Train the specialized models on denoised datasets (Th=0.8)
        for regime in regimes:
            train_path = f"step43_selective_3d_routing/data/train_ranker_v43_{regime}_th0.8.csv"
        ...
        for regime in regimes:
            train_path = f"step43_selective_3d_routing/data/train_ranker_v43_{regime}_th0.8.csv"
            val_path = f"step43_selective_3d_routing/data/train_ranker_v43_{regime}_th0.0.csv"
        ...
        combined_df_final.to_csv("step43_selective_3d_routing/data/oof_predictions_final.csv", index=False)
        print("\nSaved OOF predictions to step43_selective_3d_routing/data/oof_predictions_final.csv")
        
        overall_hit, best_cands, breakdowns = evaluate_predictions(combined_df_final)
        
        success_msg = (
            f"✅ Finished: [Step 43 train_automl.py] Success!\n"
            f"Overall Selective 3D Routing Leak-Free OOF Hit@1cm: **{overall_hit:.4%}**\n"
            f"Regime Breakdown:\n"
        )
        ...
        error_msg = f"❌ Failed: [Step 43 train_automl.py] ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
