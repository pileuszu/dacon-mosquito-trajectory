...
def main():
    try:
        msg = "🚀 Started: [Step 45 train_automl.py] Training Hybrid Classifier-Regressor specialized rankers (Th=0.8)..."
        ...
        for g_name, g_type in groups:
            train_path = f"step45_hybrid_ranker/data/train_ranker_v45_{g_name}_th0.8.csv"
            ...
            if g_type == "classification":
                # Drop regression target and tracking columns
                train_df = df_train.drop(columns=['reg_target', 'id', 'weight'])
        ...
        for g_name, g_type in groups:
            train_path = f"step45_hybrid_ranker/data/train_ranker_v45_{g_name}_th0.8.csv"
            val_path = f"step45_hybrid_ranker/data/train_ranker_v45_{g_name}_th0.0.csv"
        ...
        combined_df_final.to_csv("step45_hybrid_ranker/data/oof_predictions_final.csv", index=False)
        print("\nSaved OOF predictions to step45_hybrid_ranker/data/oof_predictions_final.csv")
        
        overall_hit, best_cands, breakdowns = evaluate_predictions(combined_df_final)
        
        success_msg = (
            f"✅ Finished: [Step 45 train_automl.py] Success!\n"
            f"Overall Hybrid OOF Hit@1cm: **{overall_hit:.4%}**\n"
            f"Regime Breakdown:\n"
        )
        ...
        error_msg = f"❌ Failed: [Step 45 train_automl.py] ERROR:\n{str(e)}\n\n{traceback.format_exc()}"
