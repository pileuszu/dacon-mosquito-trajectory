...
def test_oof_blending():
    ...
    # We will test on FAST_TURNING as it showed a huge improvement in OOF (56.84%) but represent a high-variance turning regime
    r = "fast_turning"
    print(f"=== Testing OOF Blending on Regime: {r.upper()} ===")
    
    train_data_path = f"step38_2regime_regression/train_ranker_v38_{r}.csv"
    df = pd.read_csv(train_data_path)
    
    model_path = f"step38_2regime_regression/models/ranker_v38_{r}"
    ...
    hit_rate_raw = (best_cands['reg_target'] <= 0.01).mean()
    print(f"  Raw minimum predicted distance selection Hit@1.0cm: {hit_rate_raw:.4%}")
    
    # 2. Test Blending with different sigma_dist parameters
    unique_ids = df['id'].unique()
    
    # To run quickly, let's sample 500 trajectories from unique_ids
    np.random.seed(42)
    sample_ids = np.random.choice(unique_ids, min(500, len(unique_ids)), replace=False)
    ...
    for fid in sample_ids:
        ...
        cands, tangent, S_grid, p_sacc = make_candidates_vectorized(xyz, priors, regime=r)
        ...
        # We need to map cand_indices back to coordinate indices
        # Since not all candidates are present in downsampled DF, we map:
        pred_map = {idx: dist for idx, dist in zip(cand_indices, pred_dists)}
        
        # For candidates not in the downsampled DF, we fill with a large distance (e.g. 10.0m)
    ...
    print(f"\nResults on sample of {len(sample_ids)} trajectories:")
    print(f"  Raw Selection Hit@1.0cm        : {np.mean(results['raw']):.2%}")
    print(f"  Blending (sigma_dist=0.015) Hit: {np.mean(results['blend_0.015']):.2%}")
    print(f"  Blending (sigma_dist=0.005) Hit: {np.mean(results['blend_0.005']):.2%}")
    print(f"  Blending (sigma_dist=0.002) Hit: {np.mean(results['blend_0.002']):.2%}")
    print(f"  Blending (sigma_dist=0.0005) Hit: {np.mean(results['blend_0.0005']):.2%}")
