...
from step40_dual_specialized.physics import make_candidates, extract_multi_scale_derivatives, EPS
...
# 3-Regime Grouping:
# 1. Cruising: Cluster 1
# 2. Gliding: Cluster 0, 3
# 3. Steering: Cluster 2, 4, 5
...
def prepare_step40_data_with_params(denoise_threshold=0.95):
    print(f"\n=== Step 40: Preparing 3-Regime Datasets with Denoising Threshold = {denoise_threshold} ===")
    ...
    out_dir = Path("step40_dual_specialized/data")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    for r_name, rows in regime_rows.items():
        df_reg = pd.DataFrame(rows)
        save_path = out_dir / f"train_ranker_v40_{r_name}_th{denoise_threshold}.csv"
...
def main():
    ...
    prepare_step40_data_with_params(denoise_threshold=0.0)
    ...
    for th in thresholds:
        ...
        for r_name in regimes:
            full_path = out_dir / f"train_ranker_v40_{r_name}_th0.0.csv"
            ...
            save_path = out_dir / f"train_ranker_v40_{r_name}_th{th}.csv"
