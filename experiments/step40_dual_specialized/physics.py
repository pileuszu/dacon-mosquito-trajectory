...
def make_candidates(x, priors, end_idx=-1, horizon=2, regime=None):
    ...
    # 6-Regime Dynamic Scaling & Smoothing Window Routing (With Expanded scale limits)
    if regime == "fast_straight_low":
        ...
        # Expanded maximum S_grid from 2.5 to 3.2 to resolve longitudinal lockout
        acc_smooth = acc
        if len(x_sliced) >= 5:
            x_w5 = x_sliced[-5:]
            coeffs_w5 = PINV_W5_QUAD @ x_w5
        ...
        # Expanded maximum S_grid from 3.5 to 4.2 to resolve lateral lockout in high-speed turns
        x_w3 = x_sliced[-3:]
        coeffs_w3 = PINV_W3_QUAD @ x_w3
        ...
        # Expanded maximum S_grid from 3.5 to 4.0 to resolve lockout in 제자리 turns
        x_w3 = x_sliced[-3:]
        coeffs_w3 = PINV_W3_QUAD @ x_w3
        acc_smooth = 2.0 * coeffs_w3[0]
    ...
    # INJECT RAW ACCELERATION CANDIDATES to resolve extreme turn lockout
    if regime in ["fast_extreme_turning", "slow_extreme_turning"]:
        ...
        cands_list_raw = [replace(spec, par=spec.par * S_grid, perp=spec.perp * S_grid) for spec in CANDIDATES_RAW_ACCEL_TURNING]
