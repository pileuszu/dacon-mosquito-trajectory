...
def main():
    send_discord_notification(None, "🚀 Started: [Step 67 inference_v3.py] Generating final submission coordinates with 5-Fold soft ensemble & Anisotropic Blending (V3)...")
    ...
    try:
        ...
        sigma_L = 0.015 # Longitudinal width (1.5 cm) - V3 Tuned
        sigma_T = 0.025 # Transverse width (2.5 cm) - V3 Tuned
        ...
        for s_idx in range(len(test_ids)):
            ...
            w = np.exp(-preds / 0.3) # temp = 0.3 - V3 Tuned
        ...
        sub_path = out_dir / "submission_v3.csv"
        ...
        success_msg = f"✅ Finished: [Step 67 V3] Inference completed successfully. Mean displacement: {diffs.mean()*100:.3f}cm, Max: {diffs.max()*100:.3f}cm"
        send_discord_notification(None, success_msg)
        
    except Exception as e:
        error_msg = f"❌ Failed: [Step 67 V3] Inference failed.\nError: {str(e)}\n\n{traceback.format_exc()}"
