...
from step36_four_regime.physics import (
...
from step36_four_regime.prepare_data import extract_context_features, CLUSTER_FEATURES
...
def run_step36_inference(batch_size=250, model_type="l3"):
    try:
        msg = f"🚀 [Step 36] GMM 4-Regime Custom Specialized Inference Started (batch_size={batch_size}, model_type={model_type})..."
        ...
        # Load the 4 TabularPredictors from step36
        predictors = {}
        for regime in ["slow_straight", "fast_straight", "slow_extreme_turning", "fast_turning"]:
            model_path = Path(f"step36_four_regime/models/ranker_v36_{regime}")
        ...
        print(f"Running batched inference (batch_size={batch_size}, model_type={model_type}) with Anisotropic Gaussian Blending...")
        ...
        for i in range(0, len(test_ids), batch_size):
            ...
            # Predict scores using the routed GMM regimes
        ...
        out_path = Path("outputs/step36_four_regime/submission.csv")
        ...
        success_msg = (
            f"✅ [Step 36] GMM 4-Regime Custom Specialized Inference Finished!\n"
            f"Saved to: `{out_path}`\n"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Step 36 GMM 4-Regime Inference")
    ...
    run_step36_inference(batch_size=args.batch_size, model_type=args.model_type)
